...
# Bootstrap database prerequisites (schemas, roles, auth functions)
...
```bash
# Bootstrap prerequisites (first-time or after branch reset)
pnpm --filter afenda-database db:bootstrap

# Generate migration from schema changes
pnpm --filter afenda-database db:generate

# Apply migrations (uses direct non-pooled connection)
pnpm --filter afenda-database db:migrate
