...
#### Database Migration Infrastructure (v3.1)

- **NEW**: `bootstrap-neon.sql` — Idempotent bootstrap for Neon Serverless
  Postgres prerequisites (auth schema, 8 auth functions, 4 roles, grants)
- **NEW**: `bootstrap-neon.mjs` — Programmatic runner using
  `@neondatabase/serverless` WebSocket Pool
- **NEW**: `run-migrate.mjs` — Migration runner using WebSocket Pool (not
  neon-http, which lacks transaction rollback)
- **NEW**: `0000_0000_baseline.sql` — 148-table baseline migration with
  composite PKs, RLS policies, CHECK constraints
- **NEW**: `RLS-CAST-01` gate — Prevents `auth.org_id()` without `::uuid` cast
  in RLS policies (prevents `text = uuid` operator mismatch at runtime)
- **NEW**: `PREREQ-01` gate — Validates `bootstrap-neon.sql` covers all required
  schemas, auth functions, roles, and grants
- **NEW**: `MIG-DRZ-06` online gate — Full bootstrap + migrate integration test
  on ephemeral Neon branch (creates branch, bootstraps, migrates, validates
  tables + RLS + roles, cleans up)
- **NEW**: `db:bootstrap` npm script in `afenda-database`

### Fixed

- **FIX**: `auth.org_id()` now explicitly cast to `::uuid` in all RLS policy
  expressions (tenant-policy.ts + 13 schema files) — fixes Postgres
  `operator does not exist: text = uuid` at policy evaluation
- **FIX**: `org_id <> ''` CHECK constraint replaced with
  `org_id <> '00000000-0000-0000-0000-000000000000'::uuid` across 134 schema
  files — text comparison invalid for UUID columns
- **FIX**: `docStatusEnum` (pgEnum) added to drizzle config schema glob so
  `drizzle-kit generate` includes the enum type
- **FIX**: `custom_fields` uniqueIndex changed to inline `unique()` constraint
  so FK target exists at CREATE TABLE time (before ALTER TABLE)

### Removed

- **REMOVED**: `docs/ENTITY_SCHEMA_MIGRATION_GUIDE.md` — referenced non-existent
  migration `0043_add_generated_entities.sql` (superseded by baseline)
- **REMOVED**: One-time refactor scripts (`scripts/refactor-types.ps1`,
  `scripts/refactor-types-enhanced.ps1`, `scripts/execute-refactor.ps1`,
  `scripts/fix-errors-batch.mjs`, `scripts/fix-unused.mjs`)
- **REMOVED**: `tools/scripts/migrate-to-catalog.ts` — catalog migration complete
- **REMOVED**: Broken references to non-existent SPRINT-N-COMPLETE.md files
