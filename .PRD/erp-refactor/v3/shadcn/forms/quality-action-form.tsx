...
// Form for Quality Action
// Generated from Canon schema — do not edit manually

import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import type { QualityAction } from "../types/quality-action.js";
import { QualityActionInsertSchema } from "../types/quality-action.js";
...
export function QualityActionForm({ initialData = {}, onSubmit, mode, isLoading }: QualityActionFormProps) {
  const form = useForm<Partial<QualityAction>>({
    resolver: zodResolver(QualityActionInsertSchema),
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <h2 className="text-2xl font-bold tracking-tight">
          {mode === "edit" ? "Quality Action" : "New Quality Action"}
        </h2>
            <FormField control={form.control} name="corrective_preventive" render={({ field: f }) => (
              <FormItem>
                <FormLabel className="">Corrective/Preventive</FormLabel>
            ...
            <FormField control={form.control} name="review" render={({ field: f }) => (
              <FormItem>
                <FormLabel className="">Review (→ Quality Review)</FormLabel>
                <FormControl>
                  <Input type="text" placeholder="Search Quality Review..." {...f} value={(f.value as string) ?? ""} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )} />
            <FormField control={form.control} name="feedback" render={({ field: f }) => (
              <FormItem>
                <FormLabel className="">Feedback (→ Quality Feedback)</FormLabel>
                <FormControl>
                  <Input type="text" placeholder="Search Quality Feedback..." {...f} value={(f.value as string) ?? ""} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )} />
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <FormLabel className="">Resolutions</FormLabel>
              <div className="mt-1 border rounded-md p-4 text-sm text-muted-foreground">
                {/* Child table: Quality Action Resolution — integrate with DataTable */}
                <p>Child table for Quality Action Resolution</p>
              </div>
            </div>
          </div>
        </CardContent>
