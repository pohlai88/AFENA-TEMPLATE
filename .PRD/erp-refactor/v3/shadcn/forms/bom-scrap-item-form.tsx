...
// Form for BOM Scrap Item
// Generated from Canon schema — do not edit manually

import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import type { BomScrapItem } from "../types/bom-scrap-item.js";
import { BomScrapItemInsertSchema } from "../types/bom-scrap-item.js";
...
interface BomScrapItemFormProps {
  initialData?: Partial<BomScrapItem>;
  onSubmit: (data: Partial<BomScrapItem>) => void;
  mode: "create" | "edit";
  isLoading?: boolean;
}

export function BomScrapItemForm({ initialData = {}, onSubmit, mode, isLoading }: BomScrapItemFormProps) {
  const form = useForm<Partial<BomScrapItem>>({
    resolver: zodResolver(BomScrapItemInsertSchema),
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <h2 className="text-2xl font-bold tracking-tight">
          {mode === "edit" ? "BOM Scrap Item" : "New BOM Scrap Item"}
        </h2>
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            ...
            <FormField control={form.control} name="base_amount" render={({ field: f }) => (
              <FormItem>
                <FormLabel className="">Basic Amount (Company Currency)</FormLabel>
                <FormControl>
                  <Input type="number" step="any" {...f} value={f.value != null ? Number(f.value) : ""}
                    onChange={e => f.onChange(e.target.value ? parseFloat(e.target.value) : undefined)} disabled />
                </FormControl>
