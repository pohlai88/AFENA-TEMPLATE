...
// Form for Sales Taxes and Charges
// Generated from Canon schema — do not edit manually

import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import type { SalesTaxesAndCharges } from "../types/sales-taxes-and-charges.js";
import { SalesTaxesAndChargesInsertSchema } from "../types/sales-taxes-and-charges.js";
...
export function SalesTaxesAndChargesForm({ initialData = {}, onSubmit, mode, isLoading }: SalesTaxesAndChargesFormProps) {
  const form = useForm<Partial<SalesTaxesAndCharges>>({
    resolver: zodResolver(SalesTaxesAndChargesInsertSchema),
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <h2 className="text-2xl font-bold tracking-tight">
          {mode === "edit" ? "Sales Taxes and Charges" : "New Sales Taxes and Charges"}
        </h2>
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            ...
            <FormField control={form.control} name="base_tax_amount_after_discount_amount" render={({ field: f }) => (
              <FormItem>
                <FormLabel className="">Tax Amount After Discount Amount (Company Currency)</FormLabel>
                <FormControl>
                  <Input type="number" step="any" {...f} value={f.value != null ? Number(f.value) : ""}
                    onChange={e => f.onChange(e.target.value ? parseFloat(e.target.value) : undefined)} disabled />
                </FormControl>
