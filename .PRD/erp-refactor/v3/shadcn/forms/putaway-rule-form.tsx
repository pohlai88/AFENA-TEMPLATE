...
// Form for Putaway Rule
// Generated from Canon schema — do not edit manually

import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import type { PutawayRule } from "../types/putaway-rule.js";
import { PutawayRuleInsertSchema } from "../types/putaway-rule.js";
...
export function PutawayRuleForm({ initialData = {}, onSubmit, mode, isLoading }: PutawayRuleFormProps) {
  const form = useForm<Partial<PutawayRule>>({
    resolver: zodResolver(PutawayRuleInsertSchema),
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-bold tracking-tight">
            {mode === "edit" ? (initialData.item_code as string) ?? "Putaway Rule" : "New Putaway Rule"}
          </h2>
        </div>
            ...
            <FormField control={form.control} name="capacity" render={({ field: f }) => (
              <FormItem>
            ...
            <FormField control={form.control} name="stock_capacity" render={({ field: f }) => (
              <FormItem>
                <FormLabel className="">Capacity in Stock UOM</FormLabel>
                <FormControl>
                  <Input type="number" step="any" {...f} value={f.value != null ? Number(f.value) : ""}
                    onChange={e => f.onChange(e.target.value ? parseFloat(e.target.value) : undefined)} disabled />
                </FormControl>
