...
// Form for Sales Invoice Item
// Generated from Canon schema — do not edit manually

import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import type { SalesInvoiceItem } from "../types/sales-invoice-item.js";
import { SalesInvoiceItemInsertSchema } from "../types/sales-invoice-item.js";
...
export function SalesInvoiceItemForm({ initialData = {}, onSubmit, mode, isLoading }: SalesInvoiceItemFormProps) {
  const form = useForm<Partial<SalesInvoiceItem>>({
    resolver: zodResolver(SalesInvoiceItemInsertSchema),
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <h2 className="text-2xl font-bold tracking-tight">
          {mode === "edit" ? "Sales Invoice Item" : "New Sales Invoice Item"}
        </h2>
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField control={form.control} name="delivered_by_supplier" render={({ field: f }) => (
              <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                ...
                <div className="space-y-1 leading-none">
                  <FormLabel className="">Delivered By Supplier</FormLabel>
                </div>
              </FormItem>
            )} />
          </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            ...
            <FormField control={form.control} name="discount_account" render={({ field: f }) => (
              <FormItem>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Deferred Revenue</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            ...
            <FormField control={form.control} name="incoming_rate" render={({ field: f }) => (
              <FormItem>
                <FormLabel className="">Incoming Rate (Costing)</FormLabel>
                <FormControl>
                  <Input type="number" step="any" {...f} value={f.value != null ? Number(f.value) : ""}
                    onChange={e => f.onChange(e.target.value ? parseFloat(e.target.value) : undefined)} />
                </FormControl>
            ...
            <FormField control={form.control} name="actual_batch_qty" render={({ field: f }) => (
              <FormItem>
                <FormLabel className="">Available Batch Qty at Warehouse</FormLabel>
                <FormControl>
                  <Input type="number" step="any" {...f} value={f.value != null ? Number(f.value) : ""}
                    onChange={e => f.onChange(e.target.value ? parseFloat(e.target.value) : undefined)} disabled />
                </FormControl>
      ...
      <Card>
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            {form.getValues().use_serial_batch_fields ==== 1 && parent.update_stock ==== 1 && (
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            ...
            <FormField control={form.control} name="pos_invoice_item" render={({ field: f }) => (
              <FormItem>
                <FormLabel className="">POS Invoice Item</FormLabel>
                <FormControl>
                  <Input type="text" {...f} value={(f.value as string) ?? ""} disabled />
                </FormControl>
                <FormMessage />
