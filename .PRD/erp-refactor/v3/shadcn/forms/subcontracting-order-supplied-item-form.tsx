...
// Form for Subcontracting Order Supplied Item
// Generated from Canon schema — do not edit manually

import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import type { SubcontractingOrderSuppliedItem } from "../types/subcontracting-order-supplied-item.js";
import { SubcontractingOrderSuppliedItemInsertSchema } from "../types/subcontracting-order-supplied-item.js";
...
interface SubcontractingOrderSuppliedItemFormProps {
  initialData?: Partial<SubcontractingOrderSuppliedItem>;
  onSubmit: (data: Partial<SubcontractingOrderSuppliedItem>) => void;
  mode: "create" | "edit";
  isLoading?: boolean;
}

export function SubcontractingOrderSuppliedItemForm({ initialData = {}, onSubmit, mode, isLoading }: SubcontractingOrderSuppliedItemFormProps) {
  const form = useForm<Partial<SubcontractingOrderSuppliedItem>>({
    resolver: zodResolver(SubcontractingOrderSuppliedItemInsertSchema),
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <h2 className="text-2xl font-bold tracking-tight">
          {mode === "edit" ? "Subcontracting Order Supplied Item" : "New Subcontracting Order Supplied Item"}
        </h2>
            ...
            <FormField control={form.control} name="reserve_warehouse" render={({ field: f }) => (
              <FormItem>
                <FormLabel className="">Reserve Warehouse (→ Warehouse)</FormLabel>
                <FormControl>
      ...
      <Card>
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            ...
            <FormField control={form.control} name="supplied_qty" render={({ field: f }) => (
              <FormItem>
                <FormLabel className="">Supplied Qty</FormLabel>
                <FormControl>
                  <Input type="number" step="any" {...f} value={f.value != null ? Number(f.value) : ""}
                    onChange={e => f.onChange(e.target.value ? parseFloat(e.target.value) : undefined)} disabled />
                </FormControl>
