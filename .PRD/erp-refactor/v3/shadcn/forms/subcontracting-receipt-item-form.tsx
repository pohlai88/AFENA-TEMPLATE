...
// Form for Subcontracting Receipt Item
// Generated from Canon schema — do not edit manually

import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import type { SubcontractingReceiptItem } from "../types/subcontracting-receipt-item.js";
import { SubcontractingReceiptItemInsertSchema } from "../types/subcontracting-receipt-item.js";
...
export function SubcontractingReceiptItemForm({ initialData = {}, onSubmit, mode, isLoading }: SubcontractingReceiptItemFormProps) {
  const form = useForm<Partial<SubcontractingReceiptItem>>({
    resolver: zodResolver(SubcontractingReceiptItemInsertSchema),
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <h2 className="text-2xl font-bold tracking-tight">
          {mode === "edit" ? "Subcontracting Receipt Item" : "New Subcontracting Receipt Item"}
        </h2>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Received and Accepted</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            <FormField control={form.control} name="received_qty" render={({ field: f }) => (
              <FormItem>
                <FormLabel className="font-semibold">Received Quantity</FormLabel>
                <FormControl>
            ...
            <FormField control={form.control} name="qty" render={({ field: f }) => (
              <FormItem>
                <FormLabel className="">Accepted Quantity</FormLabel>
                <FormControl>
                  <Input type="number" step="any" {...f} value={f.value != null ? Number(f.value) : ""}
            ...
            <FormField control={form.control} name="rejected_qty" render={({ field: f }) => (
              <FormItem>
                <FormLabel className="">Rejected Quantity</FormLabel>
                <FormControl>
                  <Input type="number" step="any" {...f} value={f.value != null ? Number(f.value) : ""}
                    onChange={e => f.onChange(e.target.value ? parseFloat(e.target.value) : undefined)} />
                </FormControl>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Rate and Amount</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            ...
            {!form.getValues().is_scrap_item && (
            <FormField control={form.control} name="scrap_cost_per_qty" render={({ field: f }) => (
              <FormItem>
                <FormLabel className="">Scrap Cost Per Qty</FormLabel>
                <FormControl>
                  <Input type="number" step="any" {...f} value={f.value != null ? Number(f.value) : ""}
                    onChange={e => f.onChange(e.target.value ? parseFloat(e.target.value) : undefined)} disabled />
                </FormControl>
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            <FormField control={form.control} name="warehouse" render={({ field: f }) => (
              <FormItem>
                <FormLabel className="font-semibold">Accepted Warehouse (→ Warehouse)</FormLabel>
                <FormControl>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Serial and Batch Details</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
      ...
      <Card>
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            {!form.getValues().is_fixed_asset && form.getValues().use_serial_batch_fields ==== 1 && (
