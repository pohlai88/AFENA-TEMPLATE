...
// Form for BOM Explosion Item
// Generated from Canon schema — do not edit manually

import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import type { BomExplosionItem } from "../types/bom-explosion-item.js";
import { BomExplosionItemInsertSchema } from "../types/bom-explosion-item.js";
...
interface BomExplosionItemFormProps {
  initialData?: Partial<BomExplosionItem>;
  onSubmit: (data: Partial<BomExplosionItem>) => void;
  mode: "create" | "edit";
  isLoading?: boolean;
}

export function BomExplosionItemForm({ initialData = {}, onSubmit, mode, isLoading }: BomExplosionItemFormProps) {
  const form = useForm<Partial<BomExplosionItem>>({
    resolver: zodResolver(BomExplosionItemInsertSchema),
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <h2 className="text-2xl font-bold tracking-tight">
          {mode === "edit" ? "BOM Explosion Item" : "New BOM Explosion Item"}
        </h2>
      ...
      <Card>
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            ...
            <FormField control={form.control} name="qty_consumed_per_unit" render={({ field: f }) => (
              <FormItem>
                <FormLabel className="">Qty Consumed Per Unit</FormLabel>
                <FormControl>
                  <Input type="number" step="any" {...f} value={f.value != null ? Number(f.value) : ""}
                    onChange={e => f.onChange(e.target.value ? parseFloat(e.target.value) : undefined)} disabled />
                </FormControl>
            ...
            <FormField control={form.control} name="is_sub_assembly_item" render={({ field: f }) => (
              <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                <FormControl>
                  <Checkbox checked={!!f.value} onCheckedChange={f.onChange} disabled />
                </FormControl>
                <div className="space-y-1 leading-none">
                  <FormLabel className="">Is Sub Assembly Item</FormLabel>
                </div>
              </FormItem>
            )} />
          </div>
