...
// Form for Subcontracting Receipt Supplied Item
...
import { SubcontractingReceiptSuppliedItemInsertSchema } from "../types/subcontracting-receipt-supplied-item.js";
...
interface SubcontractingReceiptSuppliedItemFormProps {
  initialData?: Partial<SubcontractingReceiptSuppliedItem>;
  onSubmit: (data: Partial<SubcontractingReceiptSuppliedItem>) => void;
  mode: "create" | "edit";
  isLoading?: boolean;
}

export function SubcontractingReceiptSuppliedItemForm({ initialData = {}, onSubmit, mode, isLoading }: SubcontractingReceiptSuppliedItemFormProps) {
  const form = useForm<Partial<SubcontractingReceiptSuppliedItem>>({
    resolver: zodResolver(SubcontractingReceiptSuppliedItemInsertSchema),
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <h2 className="text-2xl font-bold tracking-tight">
          {mode === "edit" ? "Subcontracting Receipt Supplied Item" : "New Subcontracting Receipt Supplied Item"}
        </h2>
      ...
      <Card>
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            <FormField control={form.control} name="available_qty_for_consumption" render={({ field: f }) => (
              <FormItem>
                <FormLabel className="">Available Qty For Consumption</FormLabel>
                <FormControl>
                  <Input type="number" step="any" {...f} value={f.value != null ? Number(f.value) : ""}
                    onChange={e => f.onChange(e.target.value ? parseFloat(e.target.value) : undefined)} disabled />
                </FormControl>
            ...
            <FormField control={form.control} name="current_stock" render={({ field: f }) => (
              <FormItem>
                <FormLabel className="">Current Stock</FormLabel>
                <FormControl>
                  <Input type="number" step="any" {...f} value={f.value != null ? Number(f.value) : ""}
                    onChange={e => f.onChange(e.target.value ? parseFloat(e.target.value) : undefined)} disabled />
                </FormControl>
