...
describe('SubcontractingOrderServiceItem Zod validation', () => {
  const validSample = {
      "id": "TEST-SubcontractingOrderServiceItem-001",
      "owner": "Administrator",
  ...
  it('validates a correct Subcontracting Order Service Item object', () => {
    const result = SubcontractingOrderServiceItemSchema.safeParse(validSample);
    expect(result.success).toBe(true);
  });

  it('validates insert schema (omits audit fields)', () => {
    const { id, owner, creation, modified, modified_by, ...insertData } = validSample;
    const result = SubcontractingOrderServiceItemInsertSchema.safeParse(insertData);
    expect(result.success).toBe(true);
  });
  ...
  it('rejects when required field "item_code" is missing', () => {
    ...
    const result = SubcontractingOrderServiceItemSchema.safeParse(invalid);
