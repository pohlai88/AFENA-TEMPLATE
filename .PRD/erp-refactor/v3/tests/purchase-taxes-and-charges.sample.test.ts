...
describe('PurchaseTaxesAndCharges Zod validation', () => {
  const validSample = {
      "id": "TEST-PurchaseTaxesAndCharges-001",
      "owner": "Administrator",
  ...
  it('validates a correct Purchase Taxes and Charges object', () => {
    const result = PurchaseTaxesAndChargesSchema.safeParse(validSample);
    expect(result.success).toBe(true);
  });

  it('validates insert schema (omits audit fields)', () => {
    const { id, owner, creation, modified, modified_by, ...insertData } = validSample;
    const result = PurchaseTaxesAndChargesInsertSchema.safeParse(insertData);
    expect(result.success).toBe(true);
  });
  ...
  it('rejects when required field "category" is missing', () => {
    ...
    const result = PurchaseTaxesAndChargesSchema.safeParse(invalid);
