// Form scaffold for Maintenance Visit
// Generated from Canon schema — do not edit manually
import React, { useState } from 'react';
import type { MaintenanceVisit } from '../types/maintenance-visit.js';

interface MaintenanceVisitFormProps {
  initialData?: Partial<MaintenanceVisit>;
  onSubmit: (data: Partial<MaintenanceVisit>) => void;
  mode: 'create' | 'edit';
}

export function MaintenanceVisitForm({ initialData = {}, onSubmit, mode }: MaintenanceVisitFormProps) {
  const [formData, setFormData] = useState<Partial<MaintenanceVisit>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">
        {mode === 'edit' ? formData.customer_name ?? 'Maintenance Visit' : 'New Maintenance Visit'}
      </h2>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Maintenance Schedule (→ Maintenance Schedule)</label>
            <div className="relative">
              <input
                type="text"
                placeholder="Search Maintenance Schedule..."
                value={String(formData.maintenance_schedule ?? '')}
                onChange={e => {
                  handleChange('maintenance_schedule', e.target.value);
                ...
                data-fieldname="maintenance_schedule"
              />
              {/* Link indicator */}
              {formData.maintenance_schedule && (
                <button
                  type="button"
                  onClick={() => handleChange('maintenance_schedule', '')}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
                  aria-label="Clear"
                >
                  ×
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Maintenance Schedule Detail (→ Maintenance Schedule Detail)</label>
            <div className="relative">
              <input
                type="text"
                placeholder="Search Maintenance Schedule Detail..."
                value={String(formData.maintenance_schedule_detail ?? '')}
                onChange={e => {
                  handleChange('maintenance_schedule_detail', e.target.value);
                  // TODO: Implement async search for Maintenance Schedule Detail
                  // fetch(`/api/resource/Maintenance Schedule Detail?filters=[["name","like","${e.target.value}%"]]&limit=10`)
                  //   .then(r => r.json()).then(data => setSuggestions(data.data));
                }}
                className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                autoComplete="off"
                data-doctype="Maintenance Schedule Detail"
                data-fieldname="maintenance_schedule_detail"
              />
              {/* Link indicator */}
              {formData.maintenance_schedule_detail && (
                <button
                  type="button"
                  onClick={() => handleChange('maintenance_schedule_detail', '')}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
                  aria-label="Clear"
                >
                  ×
          ...
          <div>
            ...
            <input
              type="date"
              value={String(formData.mntc_date ?? '')}
              onChange={e => handleChange('mntc_date', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" required
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Maintenance Time</label>
            <input
              type="time"
              value={String(formData.mntc_time ?? '')}
              onChange={e => handleChange('mntc_time', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
        </div>
      </div>
      {/* Section: maintenance_details */}
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          {/* Child table: purposes → Maintenance Visit Purpose */}
          <div className="col-span-2">
            ...
            <div className="mt-1 border rounded overflow-hidden">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-200">
                  {(Array.isArray(formData.purposes) ? (formData.purposes as Record<string, unknown>[]) : []).map((row, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="px-3 py-2">
                        <button
                          type="button"
                          onClick={() => {
                            const rows = [...(Array.isArray(formData.purposes) ? formData.purposes : [])];
                            rows.splice(idx, 1);
                            handleChange('purposes', rows);
                          }}
              ...
              <div className="p-2 bg-gray-50 border-t">
                <button
                  type="button"
                  onClick={() => handleChange('purposes', [...(Array.isArray(formData.purposes) ? formData.purposes : []), {}])}
      ...
      <div className="border rounded-lg p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            ...
            <textarea
              value={String(formData.customer_feedback ?? '')}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Amended From (→ Maintenance Visit)</label>
            <div className="relative">
              <input
                type="text"
                placeholder="Search Maintenance Visit..."
                value={String(formData.amended_from ?? '')}
                onChange={e => {
                  handleChange('amended_from', e.target.value);
                  // TODO: Implement async search for Maintenance Visit
                  // fetch(`/api/resource/Maintenance Visit?filters=[["name","like","${e.target.value}%"]]&limit=10`)
                  //   .then(r => r.json()).then(data => setSuggestions(data.data));
                }}
                className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                autoComplete="off"
                data-doctype="Maintenance Visit"
