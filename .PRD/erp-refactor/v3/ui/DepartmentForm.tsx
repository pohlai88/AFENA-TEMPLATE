...
export function DepartmentForm({ initialData = {}, onSubmit, mode }: DepartmentFormProps) {
  const [formData, setFormData] = useState<Partial<Department>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'Department' : 'New Department'}</h2>
          <div>
            <label className="block text-sm font-medium text-gray-700">Department</label>
            <input
              type="text"
              value={String(formData.department_name ?? '')}
              onChange={e => handleChange('department_name', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" required
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Parent Department (→ Department)</label>
            <div className="relative">
              <input
                type="text"
                placeholder="Search Department..."
                value={String(formData.parent_department ?? '')}
                onChange={e => {
                  handleChange('parent_department', e.target.value);
                ...
                data-fieldname="parent_department"
              />
              {/* Link indicator */}
              {formData.parent_department && (
                <button
                  type="button"
                  onClick={() => handleChange('parent_department', '')}
