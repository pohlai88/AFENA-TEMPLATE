// List scaffold for Budget Distribution
// Generated from Canon schema — do not edit manually
import React from 'react';
import type { BudgetDistribution } from '../types/budget-distribution.js';

interface BudgetDistributionListProps {
  data: BudgetDistribution[];
  onRowClick?: (id: string) => void;
}

export function BudgetDistributionList({ data, onRowClick }: BudgetDistributionListProps) {
  return (
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {data.map((row) => (
            ...
            >
              ...
              <td className="px-4 py-3 text-sm">{String(row.percent ?? '')}</td>
            </tr>
          ))}
        </tbody>
      </table>
