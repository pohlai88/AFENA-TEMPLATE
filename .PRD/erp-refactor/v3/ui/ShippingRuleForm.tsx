// Form scaffold for Shipping Rule
...
export function ShippingRuleForm({ initialData = {}, onSubmit, mode }: ShippingRuleFormProps) {
  const [formData, setFormData] = useState<Partial<ShippingRule>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'Shipping Rule' : 'New Shipping Rule'}</h2>
          <div>
            <label className="block text-sm font-medium text-gray-700">Shipping Rule Label</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Shipping Rule Type</label>
            <select
              value={String(formData.shipping_rule_type ?? '')}
              onChange={e => handleChange('shipping_rule_type', e.target.value)}
      ...
      <div className="border rounded-lg p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Shipping Account (→ Account)</label>
            <div className="relative">
              <input
                type="text"
      ...
      {/* Section: shipping_amount_section */}
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Calculate Based On</label>
            <select
              value={String(formData.calculate_based_on ?? '')}
              onChange={e => handleChange('calculate_based_on', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            >
              <option value="">Select...</option>
      ...
      {/* Section: Shipping Rule Conditions */}
      <div className="border rounded-lg p-4 space-y-4">
        <h4 className="font-medium text-gray-700">Shipping Rule Conditions</h4>
        <div className="grid grid-cols-2 gap-4">
          {/* Child table: conditions → Shipping Rule Condition */}
          <div className="col-span-2">
            <label className="block text-sm font-medium text-gray-700">Shipping Rule Conditions</label>
            <div className="mt-1 border rounded overflow-hidden">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-200">
                  {(Array.isArray(formData.conditions) ? (formData.conditions as Record<string, unknown>[]) : []).map((row, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="px-3 py-2">
                        <button
                          type="button"
                          onClick={() => {
                            const rows = [...(Array.isArray(formData.conditions) ? formData.conditions : [])];
                            rows.splice(idx, 1);
                            handleChange('conditions', rows);
                          }}
              ...
              <div className="p-2 bg-gray-50 border-t">
                <button
                  type="button"
                  onClick={() => handleChange('conditions', [...(Array.isArray(formData.conditions) ? formData.conditions : []), {}])}
      ...
      {/* Section: Restrict to Countries */}
      <div className="border rounded-lg p-4 space-y-4">
        <h4 className="font-medium text-gray-700">Restrict to Countries</h4>
        <div className="grid grid-cols-2 gap-4">
          {/* Child table: countries → Shipping Rule Country */}
          <div className="col-span-2">
            <label className="block text-sm font-medium text-gray-700">Valid for Countries</label>
            <div className="mt-1 border rounded overflow-hidden">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
