// Form scaffold for Advance Taxes and Charges
// Generated from Canon schema — do not edit manually
import React, { useState } from 'react';
import type { AdvanceTaxesAndCharges } from '../types/advance-taxes-and-charges.js';

interface AdvanceTaxesAndChargesFormProps {
  initialData?: Partial<AdvanceTaxesAndCharges>;
  onSubmit: (data: Partial<AdvanceTaxesAndCharges>) => void;
  mode: 'create' | 'edit';
}

export function AdvanceTaxesAndChargesForm({ initialData = {}, onSubmit, mode }: AdvanceTaxesAndChargesFormProps) {
  const [formData, setFormData] = useState<Partial<AdvanceTaxesAndCharges>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'Advance Taxes and Charges' : 'New Advance Taxes and Charges'}</h2>
          <div>
            <label className="block text-sm font-medium text-gray-700">Add Or Deduct</label>
            <select
              value={String(formData.add_deduct_tax ?? '')}
              onChange={e => handleChange('add_deduct_tax', e.target.value)}
          ...
          <div>
            ...
            >
              ...
              <option value="On Paid Amount">On Paid Amount</option>
