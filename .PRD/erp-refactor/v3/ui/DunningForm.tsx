...
export function DunningForm({ initialData = {}, onSubmit, mode }: DunningFormProps) {
  const [formData, setFormData] = useState<Partial<Dunning>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">
        {mode === 'edit' ? formData.customer_name ?? 'Dunning' : 'New Dunning'}
      </h2>
          <div>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Dunning Type (→ Dunning Type)</label>
            <div className="relative">
              <input
                type="text"
                placeholder="Search Dunning Type..."
                value={String(formData.dunning_type ?? '')}
                onChange={e => {
                  handleChange('dunning_type', e.target.value);
                  // TODO: Implement async search for Dunning Type
                  // fetch(`/api/resource/Dunning Type?filters=[["name","like","${e.target.value}%"]]&limit=10`)
                  //   .then(r => r.json()).then(data => setSuggestions(data.data));
                }}
                className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                autoComplete="off"
                data-doctype="Dunning Type"
                data-fieldname="dunning_type"
              />
              {/* Link indicator */}
              {formData.dunning_type && (
                <button
                  type="button"
                  onClick={() => handleChange('dunning_type', '')}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
                  aria-label="Clear"
                >
                  ×
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Rate of Interest (%) Yearly</label>
            <input
              type="number"
              step="any"
              value={formData.rate_of_interest != null ? Number(formData.rate_of_interest) : ''}
              onChange={e => handleChange('rate_of_interest', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
        </div>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          {/* Child table: overdue_payments → Overdue Payment */}
          <div className="col-span-2">
            <label className="block text-sm font-medium text-gray-700">Overdue Payments</label>
            <div className="mt-1 border rounded overflow-hidden">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-200">
                  {(Array.isArray(formData.overdue_payments) ? (formData.overdue_payments as Record<string, unknown>[]) : []).map((row, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="px-3 py-2">
                        <button
                          type="button"
                          onClick={() => {
                            const rows = [...(Array.isArray(formData.overdue_payments) ? formData.overdue_payments : [])];
                            rows.splice(idx, 1);
                            handleChange('overdue_payments', rows);
                          }}
              ...
              <div className="p-2 bg-gray-50 border-t">
                <button
                  type="button"
                  onClick={() => handleChange('overdue_payments', [...(Array.isArray(formData.overdue_payments) ? formData.overdue_payments : []), {}])}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              type="number"
              step="any"
              value={formData.total_interest != null ? Number(formData.total_interest) : ''}
              onChange={e => handleChange('total_interest', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Dunning Fee</label>
            <input
              type="number"
              step="any"
              value={formData.dunning_fee != null ? Number(formData.dunning_fee) : ''}
              onChange={e => handleChange('dunning_fee', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Dunning Amount</label>
            <input
              type="number"
              step="any"
              value={formData.dunning_amount != null ? Number(formData.dunning_amount) : ''}
              onChange={e => handleChange('dunning_amount', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Dunning Amount (Company Currency)</label>
            <input
              type="number"
              step="any"
              value={formData.base_dunning_amount != null ? Number(formData.base_dunning_amount) : ''}
              onChange={e => handleChange('base_dunning_amount', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
        </div>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              type="text"
              value={String(formData.spacer ?? '')}
              onChange={e => handleChange('spacer', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Total Outstanding</label>
            <input
              type="number"
              step="any"
              value={formData.total_outstanding != null ? Number(formData.total_outstanding) : ''}
              onChange={e => handleChange('total_outstanding', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Amended From (→ Dunning)</label>
            <div className="relative">
              <input
                type="text"
                placeholder="Search Dunning..."
                value={String(formData.amended_from ?? '')}
                onChange={e => {
                  handleChange('amended_from', e.target.value);
                  // TODO: Implement async search for Dunning
                  // fetch(`/api/resource/Dunning?filters=[["name","like","${e.target.value}%"]]&limit=10`)
                  //   .then(r => r.json()).then(data => setSuggestions(data.data));
                }}
                className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                autoComplete="off"
                data-doctype="Dunning"
      ...
      {/* Section: address_and_contact_section */}
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            <label className="block text-sm font-medium text-gray-700">Company Address Display</label>
            <textarea
