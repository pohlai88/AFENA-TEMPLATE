// List scaffold for Holiday List
// Generated from Canon schema — do not edit manually
import React from 'react';
import type { HolidayList } from '../types/holiday-list.js';

interface HolidayListListProps {
  data: HolidayList[];
  onRowClick?: (id: string) => void;
}

export function HolidayListList({ data, onRowClick }: HolidayListListProps) {
  return (
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Total Holidays</th>
          </tr>
        </thead>
        <tbody className="bg-white divide-y divide-gray-200">
          {data.map((row) => (
            ...
            >
              ...
              <td className="px-4 py-3 text-sm">{String(row.total_holidays ?? '')}</td>
            </tr>
          ))}
        </tbody>
      </table>
