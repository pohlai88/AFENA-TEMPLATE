// Form scaffold for Maintenance Schedule Detail
...
export function MaintenanceScheduleDetailForm({ initialData = {}, onSubmit, mode }: MaintenanceScheduleDetailFormProps) {
  const [formData, setFormData] = useState<Partial<MaintenanceScheduleDetail>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'Maintenance Schedule Detail' : 'New Maintenance Schedule Detail'}</h2>
          <div>
          ...
          <div>
            ...
            <input
              type="date"
              value={String(formData.scheduled_date ?? '')}
          ...
          <div>
            ...
            <input
              type="date"
              value={String(formData.actual_date ?? '')}
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              value={String(formData.completion_status ?? '')}
              onChange={e => handleChange('completion_status', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            >
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Item Reference (→ Maintenance Schedule Item)</label>
            <div className="relative">
              <input
                type="text"
                placeholder="Search Maintenance Schedule Item..."
                value={String(formData.item_reference ?? '')}
                onChange={e => {
                  handleChange('item_reference', e.target.value);
                  // TODO: Implement async search for Maintenance Schedule Item
                  // fetch(`/api/resource/Maintenance Schedule Item?filters=[["name","like","${e.target.value}%"]]&limit=10`)
                  //   .then(r => r.json()).then(data => setSuggestions(data.data));
                }}
                className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                autoComplete="off"
                data-doctype="Maintenance Schedule Item"
                data-fieldname="item_reference"
              />
              {/* Link indicator */}
              {formData.item_reference && (
                <button
                  type="button"
                  onClick={() => handleChange('item_reference', '')}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
                  aria-label="Clear"
                >
                  ×
