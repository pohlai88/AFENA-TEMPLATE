// Form scaffold for POS Invoice Item
// Generated from Canon schema — do not edit manually
import React, { useState } from 'react';
import type { PosInvoiceItem } from '../types/pos-invoice-item.js';

interface PosInvoiceItemFormProps {
  initialData?: Partial<PosInvoiceItem>;
  onSubmit: (data: Partial<PosInvoiceItem>) => void;
  mode: 'create' | 'edit';
}

export function PosInvoiceItemForm({ initialData = {}, onSubmit, mode }: PosInvoiceItemFormProps) {
  const [formData, setFormData] = useState<Partial<PosInvoiceItem>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'POS Invoice Item' : 'New POS Invoice Item'}</h2>
          <div>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          {formData.use_serial_batch_fields ==== 0 || formData.serial_and_batch_bundle && (
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Available Qty at Warehouse</label>
            <input
              type="number"
              step="any"
