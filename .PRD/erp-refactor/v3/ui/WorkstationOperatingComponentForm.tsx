// Form scaffold for Workstation Operating Component
...
export function WorkstationOperatingComponentForm({ initialData = {}, onSubmit, mode }: WorkstationOperatingComponentFormProps) {
  const [formData, setFormData] = useState<Partial<WorkstationOperatingComponent>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'Workstation Operating Component' : 'New Workstation Operating Component'}</h2>
          <div>
            <label className="block text-sm font-medium text-gray-700">Component Name</label>
            <input
              type="text"
              value={String(formData.component_name ?? '')}
              onChange={e => handleChange('component_name', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" required
            />
          </div>
      {/* Section: section_break_ewdg */}
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          {/* Child table: accounts → Workstation Operating Component Account */}
          <div className="col-span-2">
            <label className="block text-sm font-medium text-gray-700">Component Expense Account</label>
