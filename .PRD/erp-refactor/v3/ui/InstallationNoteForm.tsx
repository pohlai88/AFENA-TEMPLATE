// Form scaffold for Installation Note
...
export function InstallationNoteForm({ initialData = {}, onSubmit, mode }: InstallationNoteFormProps) {
  const [formData, setFormData] = useState<Partial<InstallationNote>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">
        {mode === 'edit' ? formData.customer_name ?? 'Installation Note' : 'New Installation Note'}
      </h2>
      {/* Section: Installation Note */}
      <div className="border rounded-lg p-4 space-y-4">
        <h4 className="font-medium text-gray-700">Installation Note</h4>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              type="date"
              value={String(formData.inst_date ?? '')}
              onChange={e => handleChange('inst_date', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" required
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Installation Time</label>
            <input
              type="time"
              value={String(formData.inst_time ?? '')}
              onChange={e => handleChange('inst_time', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Amended From (→ Installation Note)</label>
            <div className="relative">
              <input
                type="text"
                placeholder="Search Installation Note..."
                value={String(formData.amended_from ?? '')}
                onChange={e => {
                  handleChange('amended_from', e.target.value);
                  // TODO: Implement async search for Installation Note
                  // fetch(`/api/resource/Installation Note?filters=[["name","like","${e.target.value}%"]]&limit=10`)
                  //   .then(r => r.json()).then(data => setSuggestions(data.data));
                }}
                className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                autoComplete="off"
                data-doctype="Installation Note"
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          {/* Child table: items → Installation Note Item */}
          <div className="col-span-2">
