// Form scaffold for POS Payment Method
...
export function PosPaymentMethodForm({ initialData = {}, onSubmit, mode }: PosPaymentMethodFormProps) {
  const [formData, setFormData] = useState<Partial<PosPaymentMethod>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'POS Payment Method' : 'New POS Payment Method'}</h2>
          ...
          <div className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={!!formData.allow_in_returns}
              onChange={e => handleChange('allow_in_returns', e.target.checked)}
              className="rounded border-gray-300"
            />
            <label className="text-sm font-medium text-gray-700">Allow In Returns</label>
          </div>
          <div>
