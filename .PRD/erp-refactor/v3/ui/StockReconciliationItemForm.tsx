// Form scaffold for Stock Reconciliation Item
...
export function StockReconciliationItemForm({ initialData = {}, onSubmit, mode }: StockReconciliationItemFormProps) {
  const [formData, setFormData] = useState<Partial<StockReconciliationItem>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'Stock Reconciliation Item' : 'New Stock Reconciliation Item'}</h2>
          <div>
          ...
          {!!formData.barcode && (
          <div>
            <label className="block text-sm font-medium text-gray-700">Has Item Scanned</label>
            <input
              type="text"
              value={String(formData.has_item_scanned ?? '')}
              onChange={e => handleChange('has_item_scanned', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          )}
      ...
      <div className="border rounded-lg p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          {!formData.use_serial_batch_fields && (
          <div className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={!!formData.reconcile_all_serial_batch}
              onChange={e => handleChange('reconcile_all_serial_batch', e.target.checked)}
              className="rounded border-gray-300"
            />
            <label className="text-sm font-medium text-gray-700">Reconcile All Serial Nos / Batches</label>
          </div>
          )}
          {formData.use_serial_batch_fields ==== 0 || formData.docstatus ==== 1 && (
          <div>
            <label className="block text-sm font-medium text-gray-700">Serial / Batch Bundle (→ Serial and Batch Bundle)</label>
            <div className="relative">
              <input
                type="text"
          ...
          {formData.use_serial_batch_fields ==== 0 || formData.docstatus ==== 1 && (
          <div>
            <label className="block text-sm font-medium text-gray-700">Current Serial / Batch Bundle (→ Serial and Batch Bundle)</label>
            <div className="relative">
              <input
                type="text"
                placeholder="Search Serial and Batch Bundle..."
                value={String(formData.current_serial_and_batch_bundle ?? '')}
                onChange={e => {
                  handleChange('current_serial_and_batch_bundle', e.target.value);
                  // TODO: Implement async search for Serial and Batch Bundle
                  // fetch(`/api/resource/Serial and Batch Bundle?filters=[["name","like","${e.target.value}%"]]&limit=10`)
                  //   .then(r => r.json()).then(data => setSuggestions(data.data));
                }}
                ...
                autoComplete="off"
                data-doctype="Serial and Batch Bundle"
                data-fieldname="current_serial_and_batch_bundle"
              />
              {/* Link indicator */}
              {formData.current_serial_and_batch_bundle && (
                <button
                  type="button"
                  onClick={() => handleChange('current_serial_and_batch_bundle', '')}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
                  aria-label="Clear"
                >
                  ×
      ...
      {/* Section: Before reconciliation */}
      <div className="border rounded-lg p-4 space-y-4">
        <h4 className="font-medium text-gray-700">Before reconciliation</h4>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Current Qty</label>
            <input
              type="number"
              step="any"
              value={formData.current_qty != null ? Number(formData.current_qty) : ''}
              onChange={e => handleChange('current_qty', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
            ...
            <input
              type="number"
              step="any"
              value={formData.current_amount != null ? Number(formData.current_amount) : ''}
              onChange={e => handleChange('current_amount', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Current Valuation Rate</label>
            <input
              type="number"
              step="any"
              value={formData.current_valuation_rate != null ? Number(formData.current_valuation_rate) : ''}
              onChange={e => handleChange('current_valuation_rate', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div className="col-span-2">
            <label className="block text-sm font-medium text-gray-700">Current Serial No</label>
            <textarea
              value={String(formData.current_serial_no ?? '')}
              onChange={e => handleChange('current_serial_no', e.target.value)}
              rows={4}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Quantity Difference</label>
            <input
              type="text"
              value={String(formData.quantity_difference ?? '')}
              onChange={e => handleChange('quantity_difference', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Amount Difference</label>
            <input
              type="number"
              step="any"
              value={formData.amount_difference != null ? Number(formData.amount_difference) : ''}
              onChange={e => handleChange('amount_difference', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
        </div>
