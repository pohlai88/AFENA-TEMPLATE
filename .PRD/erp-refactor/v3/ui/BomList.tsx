...
export function BomList({ data, onRowClick }: BomListProps) {
  return (
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Has Variants</th>
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {data.map((row) => (
            ...
            >
              ...
              <td className="px-4 py-3 text-sm">{String(row.total_cost ?? '')}</td>
              <td className="px-4 py-3 text-sm">{String(row.has_variants ?? '')}</td>
