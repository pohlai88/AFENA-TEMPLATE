// Form scaffold for Quality Inspection
...
export function QualityInspectionForm({ initialData = {}, onSubmit, mode }: QualityInspectionFormProps) {
  const [formData, setFormData] = useState<Partial<QualityInspection>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'Quality Inspection' : 'New Quality Inspection'}</h2>
          <div>
          ...
          <div>
            ...
            <input
              type="date"
              value={String(formData.report_date ?? '')}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Child Row Reference</label>
            <input
              type="text"
              value={String(formData.child_row_reference ?? '')}
              onChange={e => handleChange('child_row_reference', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
          ...
          <div>
            ...
            >
              ...
              <option value="Subcontracting Receipt">Subcontracting Receipt</option>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Item Serial No (→ Serial No)</label>
            <div className="relative">
              <input
                type="text"
                placeholder="Search Serial No..."
                value={String(formData.item_serial_no ?? '')}
                onChange={e => {
                  handleChange('item_serial_no', e.target.value);
                ...
                data-fieldname="item_serial_no"
              />
              {/* Link indicator */}
              {formData.item_serial_no && (
                <button
                  type="button"
                  onClick={() => handleChange('item_serial_no', '')}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
                  aria-label="Clear"
                >
                  ×
          ...
          <div>
            ...
            <input
              type="number"
              step="any"
              value={formData.sample_size != null ? Number(formData.sample_size) : ''}
              onChange={e => handleChange('sample_size', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" required
            />
          </div>
          <div>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          {/* Child table: readings → Quality Inspection Reading */}
          <div className="col-span-2">
            ...
            <div className="mt-1 border rounded overflow-hidden">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-200">
                  {(Array.isArray(formData.readings) ? (formData.readings as Record<string, unknown>[]) : []).map((row, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="px-3 py-2">
                        <button
                          type="button"
                          onClick={() => {
                            const rows = [...(Array.isArray(formData.readings) ? formData.readings : [])];
                            rows.splice(idx, 1);
                            handleChange('readings', rows);
                          }}
              ...
              <div className="p-2 bg-gray-50 border-t">
                <button
                  type="button"
                  onClick={() => handleChange('readings', [...(Array.isArray(formData.readings) ? formData.readings : []), {}])}
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Inspected By (→ User)</label>
            <div className="relative">
              <input
                type="text"
                ...
                onChange={e => {
                  handleChange('inspected_by', e.target.value);
                ...
                autoComplete="off"
                data-doctype="User"
                data-fieldname="inspected_by"
              />
              {/* Link indicator */}
              {formData.inspected_by && (
                <button
                  type="button"
                  onClick={() => handleChange('inspected_by', '')}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
                  aria-label="Clear"
                >
                  ×
          ...
          <div>
            ...
            <input
              type="text"
              value={String(formData.verified_by ?? '')}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Amended From (→ Quality Inspection)</label>
            <div className="relative">
              <input
                type="text"
