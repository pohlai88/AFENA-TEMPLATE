// Form scaffold for Process Payment Reconciliation Log Allocations
...
export function ProcessPaymentReconciliationLogAllocationsForm({ initialData = {}, onSubmit, mode }: ProcessPaymentReconciliationLogAllocationsFormProps) {
  const [formData, setFormData] = useState<Partial<ProcessPaymentReconciliationLogAllocations>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'Process Payment Reconciliation Log Allocations' : 'New Process Payment Reconciliation Log Allocations'}</h2>
          <div>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Unreconciled Amount</label>
            <input
              type="number"
              step="any"
              value={formData.unreconciled_amount != null ? Number(formData.unreconciled_amount) : ''}
              onChange={e => handleChange('unreconciled_amount', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              type="date"
              value={String(formData.gain_loss_posting_date ?? '')}
              onChange={e => handleChange('gain_loss_posting_date', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
            ...
            <div className="relative">
              <input
                ...
                value={String(formData.difference_account ?? '')}
                onChange={e => {
                  handleChange('difference_account', e.target.value);
                ...
                data-fieldname="difference_account"
              />
              {/* Link indicator */}
              {formData.difference_account && (
                <button
                  type="button"
                  onClick={() => handleChange('difference_account', '')}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
                  aria-label="Clear"
                >
                  ×
