// Form scaffold for Master Production Schedule Item
// Generated from Canon schema — do not edit manually
import React, { useState } from 'react';
import type { MasterProductionScheduleItem } from '../types/master-production-schedule-item.js';

interface MasterProductionScheduleItemFormProps {
  initialData?: Partial<MasterProductionScheduleItem>;
  onSubmit: (data: Partial<MasterProductionScheduleItem>) => void;
  mode: 'create' | 'edit';
}

export function MasterProductionScheduleItemForm({ initialData = {}, onSubmit, mode }: MasterProductionScheduleItemFormProps) {
  const [formData, setFormData] = useState<Partial<MasterProductionScheduleItem>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'Master Production Schedule Item' : 'New Master Production Schedule Item'}</h2>
          <div>
          ...
          <div>
            ...
            <input
              type="number"
              step="1"
              value={formData.cumulative_lead_time != null ? Number(formData.cumulative_lead_time) : ''}
              onChange={e => handleChange('cumulative_lead_time', e.target.value ? parseInt(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
            ...
            <input
              type="date"
              value={String(formData.order_release_date ?? '')}
              onChange={e => handleChange('order_release_date', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
