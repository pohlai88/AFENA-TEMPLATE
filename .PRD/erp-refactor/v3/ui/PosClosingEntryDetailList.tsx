// List scaffold for POS Closing Entry Detail
// Generated from Canon schema — do not edit manually
import React from 'react';
import type { PosClosingEntryDetail } from '../types/pos-closing-entry-detail.js';

interface PosClosingEntryDetailListProps {
  data: PosClosingEntryDetail[];
  onRowClick?: (id: string) => void;
}

export function PosClosingEntryDetailList({ data, onRowClick }: PosClosingEntryDetailListProps) {
  return (
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {data.map((row) => (
            ...
            >
              ...
              <td className="px-4 py-3 text-sm">{String(row.opening_amount ?? '')}</td>
              <td className="px-4 py-3 text-sm">{String(row.expected_amount ?? '')}</td>
              <td className="px-4 py-3 text-sm">{String(row.closing_amount ?? '')}</td>
              <td className="px-4 py-3 text-sm">{String(row.difference ?? '')}</td>
            </tr>
          ))}
        </tbody>
      </table>
