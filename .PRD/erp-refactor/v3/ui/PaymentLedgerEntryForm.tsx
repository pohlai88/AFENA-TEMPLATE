// Form scaffold for Payment Ledger Entry
...
export function PaymentLedgerEntryForm({ initialData = {}, onSubmit, mode }: PaymentLedgerEntryFormProps) {
  const [formData, setFormData] = useState<Partial<PaymentLedgerEntry>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'Payment Ledger Entry' : 'New Payment Ledger Entry'}</h2>
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Amount in Account Currency</label>
            <input
              type="number"
              step="any"
              value={formData.amount_in_account_currency != null ? Number(formData.amount_in_account_currency) : ''}
              onChange={e => handleChange('amount_in_account_currency', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
