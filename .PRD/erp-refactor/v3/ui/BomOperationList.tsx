// List scaffold for BOM Operation
...
export function BomOperationList({ data, onRowClick }: BomOperationListProps) {
  return (
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Sequence ID</th>
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">FG / Semi FG Item</th>
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Qty to Produce</th>
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {data.map((row) => (
            ...
            >
              ...
              <td className="px-4 py-3 text-sm">{String(row.sequence_id ?? '')}</td>
              <td className="px-4 py-3 text-sm">{String(row.finished_good ?? '')}</td>
              <td className="px-4 py-3 text-sm">{String(row.finished_good_qty ?? '')}</td>
