// Form scaffold for Stock Closing Balance
// Generated from Canon schema — do not edit manually
import React, { useState } from 'react';
import type { StockClosingBalance } from '../types/stock-closing-balance.js';

interface StockClosingBalanceFormProps {
  initialData?: Partial<StockClosingBalance>;
  onSubmit: (data: Partial<StockClosingBalance>) => void;
  mode: 'create' | 'edit';
}

export function StockClosingBalanceForm({ initialData = {}, onSubmit, mode }: StockClosingBalanceFormProps) {
  const [formData, setFormData] = useState<Partial<StockClosingBalance>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'Stock Closing Balance' : 'New Stock Closing Balance'}</h2>
          <div>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Qty Change</label>
            <input
              type="number"
              step="any"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Balance Stock Value</label>
            <input
              type="number"
              step="any"
              value={formData.stock_value != null ? Number(formData.stock_value) : ''}
              onChange={e => handleChange('stock_value', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Stock Closing Entry (→ Stock Closing Entry)</label>
            <div className="relative">
              <input
                type="text"
                placeholder="Search Stock Closing Entry..."
                value={String(formData.stock_closing_entry ?? '')}
                onChange={e => {
                  handleChange('stock_closing_entry', e.target.value);
                  // TODO: Implement async search for Stock Closing Entry
                  // fetch(`/api/resource/Stock Closing Entry?filters=[["name","like","${e.target.value}%"]]&limit=10`)
                  //   .then(r => r.json()).then(data => setSuggestions(data.data));
                }}
                className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                autoComplete="off"
                data-doctype="Stock Closing Entry"
                data-fieldname="stock_closing_entry"
              />
              {/* Link indicator */}
              {formData.stock_closing_entry && (
                <button
                  type="button"
                  onClick={() => handleChange('stock_closing_entry', '')}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
                  aria-label="Clear"
                >
                  ×
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            <label className="block text-sm font-medium text-gray-700">Inventory Dimension key</label>
            <textarea
              value={String(formData.inventory_dimension_key ?? '')}
              onChange={e => handleChange('inventory_dimension_key', e.target.value)}
              rows={4}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
      ...
      {/* Section: Stock Ageing */}
      <div className="border rounded-lg p-4 space-y-4">
        <h4 className="font-medium text-gray-700">Stock Ageing</h4>
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            <label className="block text-sm font-medium text-gray-700">FIFO Queue</label>
            <textarea
              value={String(formData.fifo_queue ?? '')}
              onChange={e => handleChange('fifo_queue', e.target.value)}
              rows={4}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
