// Form scaffold for Asset Value Adjustment
...
export function AssetValueAdjustmentForm({ initialData = {}, onSubmit, mode }: AssetValueAdjustmentFormProps) {
  const [formData, setFormData] = useState<Partial<AssetValueAdjustment>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">
        {mode === 'edit' ? formData.asset ?? 'Asset Value Adjustment' : 'New Asset Value Adjustment'}
      </h2>
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Amended From (→ Asset Value Adjustment)</label>
            <div className="relative">
              <input
                type="text"
                placeholder="Search Asset Value Adjustment..."
                value={String(formData.amended_from ?? '')}
                onChange={e => {
                  handleChange('amended_from', e.target.value);
                  // TODO: Implement async search for Asset Value Adjustment
                  // fetch(`/api/resource/Asset Value Adjustment?filters=[["name","like","${e.target.value}%"]]&limit=10`)
                  //   .then(r => r.json()).then(data => setSuggestions(data.data));
                }}
                className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                autoComplete="off"
                data-doctype="Asset Value Adjustment"
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <h4 className="font-medium text-gray-700">Value Details</h4>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Current Asset Value</label>
            <input
              type="number"
              step="any"
              value={formData.current_asset_value != null ? Number(formData.current_asset_value) : ''}
              onChange={e => handleChange('current_asset_value', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" required
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">New Asset Value</label>
            <input
              type="number"
              step="any"
              value={formData.new_asset_value != null ? Number(formData.new_asset_value) : ''}
              onChange={e => handleChange('new_asset_value', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" required
            />
          </div>
          <div>
