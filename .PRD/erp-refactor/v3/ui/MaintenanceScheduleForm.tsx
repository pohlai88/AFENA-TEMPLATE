// Form scaffold for Maintenance Schedule
...
export function MaintenanceScheduleForm({ initialData = {}, onSubmit, mode }: MaintenanceScheduleFormProps) {
  const [formData, setFormData] = useState<Partial<MaintenanceSchedule>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'Maintenance Schedule' : 'New Maintenance Schedule'}</h2>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          {/* Child table: items → Maintenance Schedule Item */}
      ...
      <div className="border rounded-lg p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          {/* Child table: schedules → Maintenance Schedule Detail */}
          <div className="col-span-2">
            ...
            <div className="mt-1 border rounded overflow-hidden">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-200">
                  {(Array.isArray(formData.schedules) ? (formData.schedules as Record<string, unknown>[]) : []).map((row, idx) => (
                    <tr key={idx}>
                      <td className="px-3 py-2 text-sm text-gray-500">{idx + 1}</td>
                      <td className="px-3 py-2 text-sm">{JSON.stringify(row)}</td>
                      <td className="px-3 py-2">
                        <button
                          type="button"
                          onClick={() => {
                            const rows = [...(Array.isArray(formData.schedules) ? formData.schedules : [])];
                            rows.splice(idx, 1);
                            handleChange('schedules', rows);
                          }}
                          className="text-red-500 hover:text-red-700 text-sm"
                        >
                          Remove
              ...
              <div className="p-2 bg-gray-50 border-t">
                <button
                  type="button"
                  onClick={() => handleChange('schedules', [...(Array.isArray(formData.schedules) ? formData.schedules : []), {}])}
                  className="text-sm text-blue-600 hover:text-blue-800"
                >
                  + Add Row
                </button>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Amended From (→ Maintenance Schedule)</label>
            <div className="relative">
              <input
                type="text"
                ...
                onChange={e => {
                  ...
                  // TODO: Implement async search for Maintenance Schedule
                  // fetch(`/api/resource/Maintenance Schedule?filters=[["name","like","${e.target.value}%"]]&limit=10`)
                  //   .then(r => r.json()).then(data => setSuggestions(data.data));
                }}
                className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                autoComplete="off"
                data-doctype="Maintenance Schedule"
