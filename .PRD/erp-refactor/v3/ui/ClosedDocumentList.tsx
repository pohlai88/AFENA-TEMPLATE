// List scaffold for Closed Document
// Generated from Canon schema — do not edit manually
import React from 'react';
import type { ClosedDocument } from '../types/closed-document.js';

interface ClosedDocumentListProps {
  data: ClosedDocument[];
  onRowClick?: (id: string) => void;
}

export function ClosedDocumentList({ data, onRowClick }: ClosedDocumentListProps) {
  return (
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {data.map((row) => (
            ...
            >
              ...
              <td className="px-4 py-3 text-sm">{String(row.closed ?? '')}</td>
            </tr>
          ))}
        </tbody>
      </table>
