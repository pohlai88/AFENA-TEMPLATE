// List scaffold for Production Plan Item
// Generated from Canon schema — do not edit manually
import React from 'react';
import type { ProductionPlanItem } from '../types/production-plan-item.js';

interface ProductionPlanItemListProps {
  data: ProductionPlanItem[];
  onRowClick?: (id: string) => void;
}

export function ProductionPlanItemList({ data, onRowClick }: ProductionPlanItemListProps) {
  return (
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Finished Goods Warehouse</th>
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Planned Start Date</th>
          </tr>
        </thead>
        <tbody className="bg-white divide-y divide-gray-200">
          {data.map((row) => (
            ...
            >
              ...
              <td className="px-4 py-3 text-sm">{String(row.planned_start_date ?? '')}</td>
            </tr>
          ))}
        </tbody>
      </table>
