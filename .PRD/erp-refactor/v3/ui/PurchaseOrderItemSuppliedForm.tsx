// Form scaffold for Purchase Order Item Supplied
// Generated from Canon schema — do not edit manually
import React, { useState } from 'react';
import type { PurchaseOrderItemSupplied } from '../types/purchase-order-item-supplied.js';

interface PurchaseOrderItemSuppliedFormProps {
  initialData?: Partial<PurchaseOrderItemSupplied>;
  onSubmit: (data: Partial<PurchaseOrderItemSupplied>) => void;
  mode: 'create' | 'edit';
}

export function PurchaseOrderItemSuppliedForm({ initialData = {}, onSubmit, mode }: PurchaseOrderItemSuppliedFormProps) {
  const [formData, setFormData] = useState<Partial<PurchaseOrderItemSupplied>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'Purchase Order Item Supplied' : 'New Purchase Order Item Supplied'}</h2>
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Reserve Warehouse (→ Warehouse)</label>
            <div className="relative">
              <input
                type="text"
                placeholder="Search Warehouse..."
                value={String(formData.reserve_warehouse ?? '')}
                onChange={e => {
                  handleChange('reserve_warehouse', e.target.value);
                ...
                data-fieldname="reserve_warehouse"
              />
              {/* Link indicator */}
              {formData.reserve_warehouse && (
                <button
                  type="button"
                  onClick={() => handleChange('reserve_warehouse', '')}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
                  aria-label="Clear"
                >
                  ×
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Supplied Qty</label>
            <input
              type="number"
              step="any"
              value={formData.supplied_qty != null ? Number(formData.supplied_qty) : ''}
              onChange={e => handleChange('supplied_qty', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Total Supplied Qty</label>
            <input
              type="number"
              step="any"
              value={formData.total_supplied_qty != null ? Number(formData.total_supplied_qty) : ''}
              onChange={e => handleChange('total_supplied_qty', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
        </div>
