// Form scaffold for Maintenance Visit Purpose
...
export function MaintenanceVisitPurposeForm({ initialData = {}, onSubmit, mode }: MaintenanceVisitPurposeFormProps) {
  const [formData, setFormData] = useState<Partial<MaintenanceVisitPurpose>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'Maintenance Visit Purpose' : 'New Maintenance Visit Purpose'}</h2>
          <div>
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                value={String(formData.service_person ?? '')}
                onChange={e => {
                  handleChange('service_person', e.target.value);
                ...
                data-fieldname="service_person"
              />
              {/* Link indicator */}
              {formData.service_person && (
                <button
                  type="button"
                  onClick={() => handleChange('service_person', '')}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
                  aria-label="Clear"
                >
                  ×
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            ...
            <textarea
              value={String(formData.work_done ?? '')}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Maintenance Schedule Detail</label>
            <input
              type="text"
              value={String(formData.maintenance_schedule_detail ?? '')}
              onChange={e => handleChange('maintenance_schedule_detail', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
        </div>
