// Form scaffold for Asset Capitalization Asset Item
// Generated from Canon schema — do not edit manually
import React, { useState } from 'react';
import type { AssetCapitalizationAssetItem } from '../types/asset-capitalization-asset-item.js';

interface AssetCapitalizationAssetItemFormProps {
  initialData?: Partial<AssetCapitalizationAssetItem>;
  onSubmit: (data: Partial<AssetCapitalizationAssetItem>) => void;
  mode: 'create' | 'edit';
}

export function AssetCapitalizationAssetItemForm({ initialData = {}, onSubmit, mode }: AssetCapitalizationAssetItemFormProps) {
  const [formData, setFormData] = useState<Partial<AssetCapitalizationAssetItem>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'Asset Capitalization Asset Item' : 'New Asset Capitalization Asset Item'}</h2>
          <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              type="number"
              step="any"
              value={formData.asset_value != null ? Number(formData.asset_value) : ''}
              onChange={e => handleChange('asset_value', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
        </div>
