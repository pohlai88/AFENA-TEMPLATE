// Form scaffold for Master Production Schedule
...
export function MasterProductionScheduleForm({ initialData = {}, onSubmit, mode }: MasterProductionScheduleFormProps) {
  const [formData, setFormData] = useState<Partial<MasterProductionSchedule>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'Master Production Schedule' : 'New Master Production Schedule'}</h2>
      {/* Tab: Planning */}
      ...
      {/* Section: section_break_rrrx */}
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          {/* Child table: select_items → Master Production Schedule Item */}
            ...
            <div className="mt-1 border rounded overflow-hidden">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-200">
                  {(Array.isArray(formData.select_items) ? (formData.select_items as Record<string, unknown>[]) : []).map((row, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="px-3 py-2">
                        <button
                          type="button"
                          onClick={() => {
                            const rows = [...(Array.isArray(formData.select_items) ? formData.select_items : [])];
                            rows.splice(idx, 1);
                            handleChange('select_items', rows);
                          }}
              ...
              <div className="p-2 bg-gray-50 border-t">
                <button
                  type="button"
                  onClick={() => handleChange('select_items', [...(Array.isArray(formData.select_items) ? formData.select_items : []), {}])}
      ...
      {/* Tab: Demand */}
      <div className="border-b pb-2 mb-4">
        <h3 className="text-lg font-medium">Demand</h3>
      </div>
      {/* Section: Open Orders */}
      <div className="border rounded-lg p-4 space-y-4">
        <h4 className="font-medium text-gray-700">Open Orders</h4>
        <div className="grid grid-cols-2 gap-4">
      ...
      {/* Section: section_break_xtby */}
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
        </div>
      </div>
      {/* Section: Actual Demand */}
      <div className="border rounded-lg p-4 space-y-4">
        <h4 className="font-medium text-gray-700">Actual Demand</h4>
        <div className="grid grid-cols-2 gap-4">
          {/* Child table: items → Master Production Schedule Item */}
          <div className="col-span-2">
      ...
      {/* Section: Forecast Demand */}
      <div className="border rounded-lg p-4 space-y-4">
        <h4 className="font-medium text-gray-700">Forecast Demand</h4>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Sales Forecast (→ Sales Forecast)</label>
            <div className="relative">
              <input
                type="text"
                placeholder="Search Sales Forecast..."
                value={String(formData.sales_forecast ?? '')}
                onChange={e => {
                  handleChange('sales_forecast', e.target.value);
                  // TODO: Implement async search for Sales Forecast
                  // fetch(`/api/resource/Sales Forecast?filters=[["name","like","${e.target.value}%"]]&limit=10`)
                  //   .then(r => r.json()).then(data => setSuggestions(data.data));
                }}
                className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                autoComplete="off"
                data-doctype="Sales Forecast"
                data-fieldname="sales_forecast"
              />
              {/* Link indicator */}
              {formData.sales_forecast && (
                <button
                  type="button"
                  onClick={() => handleChange('sales_forecast', '')}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
                  aria-label="Clear"
                >
                  ×
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Amended From (→ Master Production Schedule)</label>
            <div className="relative">
              <input
                type="text"
