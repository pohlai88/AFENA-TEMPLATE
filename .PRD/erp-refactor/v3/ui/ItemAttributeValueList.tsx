// List scaffold for Item Attribute Value
// Generated from Canon schema — do not edit manually
import React from 'react';
import type { ItemAttributeValue } from '../types/item-attribute-value.js';

interface ItemAttributeValueListProps {
  data: ItemAttributeValue[];
  onRowClick?: (id: string) => void;
}

export function ItemAttributeValueList({ data, onRowClick }: ItemAttributeValueListProps) {
  return (
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {data.map((row) => (
            ...
            >
              ...
              <td className="px-4 py-3 text-sm">{String(row.abbr ?? '')}</td>
            </tr>
          ))}
        </tbody>
      </table>
