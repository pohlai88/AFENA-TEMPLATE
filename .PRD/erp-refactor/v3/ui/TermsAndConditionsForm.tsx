// Form scaffold for Terms and Conditions
...
export function TermsAndConditionsForm({ initialData = {}, onSubmit, mode }: TermsAndConditionsFormProps) {
  const [formData, setFormData] = useState<Partial<TermsAndConditions>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'Terms and Conditions' : 'New Terms and Conditions'}</h2>
          <div>
      ...
      {/* Section: Applicable Modules */}
      <div className="border rounded-lg p-4 space-y-4">
        <h4 className="font-medium text-gray-700">Applicable Modules</h4>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            <label className="block text-sm font-medium text-gray-700">Terms and Conditions Help</label>
            <textarea
              value={String(formData.terms_and_conditions_help ?? '')}
              onChange={e => handleChange('terms_and_conditions_help', e.target.value)}
              rows={4}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
