// List scaffold for Item Wise Tax Detail
// Generated from Canon schema — do not edit manually
import React from 'react';
import type { ItemWiseTaxDetail } from '../types/item-wise-tax-detail.js';

interface ItemWiseTaxDetailListProps {
  data: ItemWiseTaxDetail[];
  onRowClick?: (id: string) => void;
}

export function ItemWiseTaxDetailList({ data, onRowClick }: ItemWiseTaxDetailListProps) {
  return (
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Item Row</th>
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Tax Row</th>
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {data.map((row) => (
            ...
            >
              ...
              <td className="px-4 py-3 text-sm">{String(row.item_row ?? '')}</td>
              <td className="px-4 py-3 text-sm">{String(row.tax_row ?? '')}</td>
