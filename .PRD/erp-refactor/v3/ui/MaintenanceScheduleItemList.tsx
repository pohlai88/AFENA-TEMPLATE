// List scaffold for Maintenance Schedule Item
// Generated from Canon schema — do not edit manually
import React from 'react';
import type { MaintenanceScheduleItem } from '../types/maintenance-schedule-item.js';

interface MaintenanceScheduleItemListProps {
  data: MaintenanceScheduleItem[];
  onRowClick?: (id: string) => void;
}

export function MaintenanceScheduleItemList({ data, onRowClick }: MaintenanceScheduleItemListProps) {
  return (
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">No of Visits</th>
          </tr>
        </thead>
        <tbody className="bg-white divide-y divide-gray-200">
          {data.map((row) => (
            ...
            >
              ...
              <td className="px-4 py-3 text-sm">{String(row.no_of_visits ?? '')}</td>
            </tr>
          ))}
        </tbody>
      </table>
