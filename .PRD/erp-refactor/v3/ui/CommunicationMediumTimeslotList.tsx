// List scaffold for Communication Medium Timeslot
// Generated from Canon schema — do not edit manually
import React from 'react';
import type { CommunicationMediumTimeslot } from '../types/communication-medium-timeslot.js';

interface CommunicationMediumTimeslotListProps {
  data: CommunicationMediumTimeslot[];
  onRowClick?: (id: string) => void;
}

export function CommunicationMediumTimeslotList({ data, onRowClick }: CommunicationMediumTimeslotListProps) {
  return (
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Employee Group</th>
          </tr>
        </thead>
        <tbody className="bg-white divide-y divide-gray-200">
          {data.map((row) => (
            ...
            >
              ...
              <td className="px-4 py-3 text-sm">{String(row.employee_group ?? '')}</td>
            </tr>
          ))}
        </tbody>
      </table>
