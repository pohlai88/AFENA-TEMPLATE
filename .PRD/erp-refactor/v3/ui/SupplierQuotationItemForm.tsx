// Form scaffold for Supplier Quotation Item
// Generated from Canon schema — do not edit manually
import React, { useState } from 'react';
import type { SupplierQuotationItem } from '../types/supplier-quotation-item.js';

interface SupplierQuotationItemFormProps {
  initialData?: Partial<SupplierQuotationItem>;
  onSubmit: (data: Partial<SupplierQuotationItem>) => void;
  mode: 'create' | 'edit';
}

export function SupplierQuotationItemForm({ initialData = {}, onSubmit, mode }: SupplierQuotationItemFormProps) {
  const [formData, setFormData] = useState<Partial<SupplierQuotationItem>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'Supplier Quotation Item' : 'New Supplier Quotation Item'}</h2>
          <div>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Reference Document Type</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Request for Quotation (→ Request for Quotation)</label>
            <div className="relative">
              <input
                type="text"
                placeholder="Search Request for Quotation..."
                value={String(formData.request_for_quotation ?? '')}
                onChange={e => {
                  handleChange('request_for_quotation', e.target.value);
                ...
                data-fieldname="request_for_quotation"
              />
              {/* Link indicator */}
              {formData.request_for_quotation && (
                <button
                  type="button"
                  onClick={() => handleChange('request_for_quotation', '')}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
                  aria-label="Clear"
                >
                  ×
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Request for Quotation Item</label>
            <input
              type="text"
              value={String(formData.request_for_quotation_item ?? '')}
              onChange={e => handleChange('request_for_quotation_item', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div className="col-span-2">
