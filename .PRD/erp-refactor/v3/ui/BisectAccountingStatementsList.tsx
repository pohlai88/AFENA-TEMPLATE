// List scaffold for Bisect Accounting Statements
// Generated from Canon schema — do not edit manually
import React from 'react';
import type { BisectAccountingStatements } from '../types/bisect-accounting-statements.js';

interface BisectAccountingStatementsListProps {
  data: BisectAccountingStatements[];
  onRowClick?: (id: string) => void;
}

export function BisectAccountingStatementsList({ data, onRowClick }: BisectAccountingStatementsListProps) {
  return (
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Current Node</th>
          </tr>
        </thead>
        <tbody className="bg-white divide-y divide-gray-200">
          {data.map((row) => (
            ...
            >
              ...
              <td className="px-4 py-3 text-sm">{String(row.algorithm ?? '')}</td>
              <td className="px-4 py-3 text-sm">{String(row.current_node ?? '')}</td>
            </tr>
          ))}
        </tbody>
      </table>
