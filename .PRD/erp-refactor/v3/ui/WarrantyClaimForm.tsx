// Form scaffold for Warranty Claim
...
export function WarrantyClaimForm({ initialData = {}, onSubmit, mode }: WarrantyClaimFormProps) {
  const [formData, setFormData] = useState<Partial<WarrantyClaim>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">
        {mode === 'edit' ? formData.customer_name ?? 'Warranty Claim' : 'New Warranty Claim'}
      </h2>
          <div>
          ...
          <div>
            ...
            <input
              type="date"
              value={String(formData.complaint_date ?? '')}
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            ...
            <textarea
              value={String(formData.complaint ?? '')}
      ...
      {/* Section: Item and Warranty Details */}
      <div className="border rounded-lg p-4 space-y-4">
        <h4 className="font-medium text-gray-700">Item and Warranty Details</h4>
        <div className="grid grid-cols-2 gap-4">
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Warranty / AMC Status</label>
            <select
              value={String(formData.warranty_amc_status ?? '')}
              onChange={e => handleChange('warranty_amc_status', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            >
              <option value="">Select...</option>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              type="datetime-local"
              value={String(formData.resolution_date ?? '')}
              onChange={e => handleChange('resolution_date', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Resolved By (→ User)</label>
            <div className="relative">
              <input
                type="text"
                placeholder="Search User..."
                value={String(formData.resolved_by ?? '')}
                onChange={e => {
                  handleChange('resolved_by', e.target.value);
                ...
                autoComplete="off"
                data-doctype="User"
                data-fieldname="resolved_by"
              />
              {/* Link indicator */}
              {formData.resolved_by && (
                <button
                  type="button"
                  onClick={() => handleChange('resolved_by', '')}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
                  aria-label="Clear"
                >
                  ×
      ...
      <div className="border rounded-lg p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            ...
            <textarea
              value={String(formData.service_address ?? '')}
      ...
      <div className="border rounded-lg p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              type="text"
              value={String(formData.complaint_raised_by ?? '')}
              onChange={e => handleChange('complaint_raised_by', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">From Company</label>
            <input
              type="text"
              value={String(formData.from_company ?? '')}
              onChange={e => handleChange('from_company', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Amended From (→ Warranty Claim)</label>
            <div className="relative">
              <input
                type="text"
                placeholder="Search Warranty Claim..."
                value={String(formData.amended_from ?? '')}
                onChange={e => {
                  handleChange('amended_from', e.target.value);
                  // TODO: Implement async search for Warranty Claim
                  // fetch(`/api/resource/Warranty Claim?filters=[["name","like","${e.target.value}%"]]&limit=10`)
                  //   .then(r => r.json()).then(data => setSuggestions(data.data));
                }}
                className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                autoComplete="off"
                data-doctype="Warranty Claim"
