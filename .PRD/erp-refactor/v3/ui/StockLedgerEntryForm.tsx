// Form scaffold for Stock Ledger Entry
...
export function StockLedgerEntryForm({ initialData = {}, onSubmit, mode }: StockLedgerEntryFormProps) {
  const [formData, setFormData] = useState<Partial<StockLedgerEntry>>(initialData);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-xl font-semibold">{mode === 'edit' ? 'Stock Ledger Entry' : 'New Stock Ledger Entry'}</h2>
          <div>
          ...
          <div className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={!!formData.is_adjustment_entry}
              onChange={e => handleChange('is_adjustment_entry', e.target.checked)}
              className="rounded border-gray-300"
            />
            <label className="text-sm font-medium text-gray-700">Is Adjustment Entry</label>
          </div>
          {!!formData.serial_and_batch_bundle && (
          <div className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={!!formData.auto_created_serial_and_batch_bundle}
              onChange={e => handleChange('auto_created_serial_and_batch_bundle', e.target.checked)}
              className="rounded border-gray-300"
            />
            <label className="text-sm font-medium text-gray-700">Auto Created Serial and Batch Bundle</label>
          </div>
          )}
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Dependant SLE Voucher Detail No</label>
            <input
              type="text"
              value={String(formData.dependant_sle_voucher_detail_no ?? '')}
              onChange={e => handleChange('dependant_sle_voucher_detail_no', e.target.value)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
      ...
      <div className="border rounded-lg p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={!!formData.recalculate_rate}
              onChange={e => handleChange('recalculate_rate', e.target.checked)}
              className="rounded border-gray-300"
            />
            <label className="text-sm font-medium text-gray-700">Recalculate Incoming/Outgoing Rate</label>
          </div>
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Qty After Transaction</label>
            <input
              type="number"
              step="any"
              value={formData.qty_after_transaction != null ? Number(formData.qty_after_transaction) : ''}
              onChange={e => handleChange('qty_after_transaction', e.target.value ? parseFloat(e.target.value) : undefined)}
              className="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
            />
          </div>
          <div>
