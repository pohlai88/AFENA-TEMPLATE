...
2. **Generate Migration**: `cd packages/database && pnpm drizzle-kit generate`
3. **Review SQL**: Check generated SQL in `drizzle/*.sql`
4. **Edit if Needed**: Add data migrations, RLS policies
...
# Test RLS
psql -d afenda_dev -c "SET auth.org_id = 'org_test'; SELECT * FROM items;"

# Verify indexes
psql -d afenda_dev -c "\d items"
```
