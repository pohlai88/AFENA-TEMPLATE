...
**Layer 1: Foundation** • **Role:** Structured Logging
...
Layer 1: Foundation (canon, database, logger ← YOU ARE HERE, ui)
...
- `createLogger(options)` — Create custom logger
- `createRequestLogger(context)` — Request-scoped logger

### Log Levels

- `logger.trace(obj, msg)` — Detailed debug info
- `logger.debug(obj, msg)` — Debug info
- `logger.info(obj, msg)` — Informational messages
- `logger.warn(obj, msg)` — Warnings
- `logger.error(obj, msg)` — Errors
- `logger.fatal(obj, msg)` — Fatal errors (process.exit)

### Helpers

- `redactSensitive(obj)` — Redact sensitive fields (passwords, tokens)
...
**NONE** — Logger has zero workspace dependencies.

### External Dependencies

- `pino` — Fast JSON logger
- `pino-abstract-transport` — Transport abstraction
- `pino-pretty` (peer) — Pretty-print for development

### Who Depends on This Package

- ✅ `afenda-crud` (Layer 3) — logs mutations
- ✅ `afenda-workflow` (Layer 2) — logs rule evaluation
- ✅ `afenda-advisory` (Layer 2) — logs analytics
- ✅ All 116 business-domain packages (Layer 2) — log business events
...
### 🔒 Rule 3: Use Structured Logging (JSON)
...
### 🔒 Rule 4: Use Child Loggers for Context
...
| **Can it import from domains?** | ❌ NO (upper layer) |
| **Can it import from crud?** | ❌ NO (upper layer) |
| **Can it log sensitive data?** | ❌ NO (use redactSensitive) |
