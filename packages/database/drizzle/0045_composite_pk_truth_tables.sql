...
-- Converts single-column PK (id) to composite PK (org_id, id) for all truth tables
-- 
-- Strategy:
-- 1. Add composite PK constraint WITHOUT dropping old PK (Postgres allows multiple constraints)
-- 2. Update indexes to use composite PK
-- 3. Drop old single-column PK
-- 4. Verify data integrity
--
-- Risk: Medium - requires table locks, but NOT VALID pattern minimizes downtime
-- Rollback: Keep old PK until verified, can drop composite and keep single-column
--
-- Estimated time: ~5-10 minutes for 94 truth tables (depends on data volume)

BEGIN;

-- ══════════════════════════════════════════════════════════════════════════════
-- Phase 1: Add composite PK constraints (NOT VALID for speed)
-- ══════════════════════════════════════════════════════════════════════════════
...
-- org_usage_daily
ALTER TABLE org_usage_daily DROP CONSTRAINT IF EXISTS org_usage_daily_pkey;
ALTER TABLE org_usage_daily ADD CONSTRAINT org_usage_daily_pkey PRIMARY KEY (org_id, id);
...
BEGIN
  FOR table_name IN 
    ...
    AND tablename IN (
      'addresses', 'bank_accounts', 'bank_reconciliation_sessions', 'bank_statement_lines',
      'boms', 'bom_lines', 'budgets', 'budget_commitments', 'chart_of_accounts',
      'communications', 'companies', 'contacts', 'contracts', 'cost_centers',
      'credit_notes', 'currencies', 'custom_field_sync_queue', 'custom_field_values',
      'custom_fields', 'customer_profiles', 'debit_notes', 'delivery_note_lines',
      'delivery_notes', 'discount_rules', 'entity_attachments', 'entity_view_fields',
      'entity_views', 'fiscal_periods', 'assets', 'depreciation_schedules', 'asset_events',
      'fx_rates', 'goods_receipt_lines', 'goods_receipts', 'intercompany_transactions',
      'item_groups', 'items', 'landed_cost_docs', 'landed_cost_allocations', 'lot_tracking',
      'match_results', 'meta_aliases', 'meta_alias_resolution_rules', 'meta_alias_sets',
      'meta_assets', 'meta_lineage_edges', 'meta_quality_checks', 'meta_semantic_terms',
      'meta_value_aliases', 'migration_checkpoints', 'migration_conflict_resolutions',
      'migration_conflicts', 'migration_jobs', 'migration_lineage', 'migration_merge_explanations',
      'migration_quarantine', 'migration_reports', 'migration_row_snapshots', 'number_sequences',
      'org_usage_daily', 'payment_allocations', 'payment_terms', 'payments',
      'price_list_items', 'price_lists', 'projects', 'purchase_invoice_lines',
      'purchase_invoices', 'purchase_order_lines', 'purchase_orders', 'purchase_requests',
      'quotation_lines', 'quotations', 'revenue_schedule_lines', 'revenue_schedules',
      'sales_invoice_lines', 'sales_invoices', 'sales_order_lines', 'sales_orders',
      'sites', 'stock_movements', 'supplier_profiles', 'tax_rates', 'uom', 'uom_conversions',
      'warehouses', 'webhook_deliveries', 'webhook_endpoints', 'wip_movements', 'work_orders',
      'approval_chains', 'approval_steps', 'approval_requests', 'approval_decisions'
    )
  LOOP
    EXECUTE format('SELECT COUNT(*) FROM (SELECT org_id, id, COUNT(*) FROM %I GROUP BY org_id, id HAVING COUNT(*) > 1) AS dups', table_name) INTO dup_count;
    IF dup_count > 0 THEN
      RAISE EXCEPTION 'Duplicate (org_id, id) pairs found in table %', table_name;
    END IF;
  END LOOP;
  
  RAISE NOTICE 'Verification complete: All 94 truth tables have unique (org_id, id) pairs';
END $$;
...
-- 1. Update Drizzle schema files to use composite PK:
--    primaryKey({ columns: [table.orgId, table.id] })
--
-- 2. Regenerate schema barrel: pnpm db:barrel
--
-- 3. Run schema-lint to verify Gate 2 passes: pnpm db:lint
--
-- 4. Update FK constraints that reference these tables (if any)
--
-- 5. Monitor query performance - composite PKs may affect query plans
