...
>(
  ...
  const tableName = (t.orgId as any).table?.[Symbol.for('drizzle:Name')] ?? '';
  const prefix = tableName ? `${tableName}_` : '';
  return foreignKey({
    name: `${prefix}${name}_fk`,
...
export function tenantFkIndex<T extends { orgId: PgColumn }>(
  ...
  const tableName = (t.orgId as any).table?.[Symbol.for('drizzle:Name')] ?? '';
  const prefix = tableName ? `${tableName}_` : '';
  return index(`${prefix}${name}_org_fk_idx`).on(t.orgId, fkColumn);
}
