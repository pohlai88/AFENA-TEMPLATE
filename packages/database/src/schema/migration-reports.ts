...
export const migrationReports = pgTable(
  ...
  (table) => [
    ...
    crudPolicy({
      ...
      read: sql`(select EXISTS (SELECT 1 FROM migration_jobs WHERE migration_jobs.id = ${table.jobId} AND migration_jobs.org_id = auth.org_id()))`,
      modify: sql`(select EXISTS (SELECT 1 FROM migration_jobs WHERE migration_jobs.id = ${table.jobId} AND migration_jobs.org_id = auth.org_id()))`,
    }),
  ]
);
