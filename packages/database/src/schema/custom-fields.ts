...
export const customFields = pgTable(
  ...
  (table) => [
    ...
    unique('custom_fields_org_entity_id_uniq').on(table.orgId, table.entityType, table.id),

    // CHECK constraints
    check(
      'custom_fields_org_not_empty',
      sql`org_id <> '00000000-0000-0000-0000-000000000000'::uuid`,
    ),
    check('custom_fields_field_name_snake', sql`field_name ~ '^[a-z][a-z0-9_]*$'`),
    check('custom_fields_storage_mode_chk', sql`storage_mode IN ('jsonb_only','indexed')`),
    check(
      'custom_fields_required_needs_default',
      sql`is_required = false OR default_value IS NOT NULL`,
    ),
    check('custom_fields_type_config_is_object', sql`jsonb_typeof(type_config) = 'object'`),
    check(
