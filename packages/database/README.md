...
<!-- Signature: sha256:8b5df3a1f25a7c38de533667e74e20e6af6fa69c5927d0554722790d606629c2 -->
# afenda-database
...
├── src/
    ├── __tests__/ (5 files)
    ├── ddl/ (2 files)
    ├── helpers/ — Helper functions (12 files)
    ├── observability/ (2 files)
    ├── query-plan/ (1 file)
    ├── schema/ — Database or validation schemas (161 files)
    ├── scripts/ — Build or runtime scripts (17 files)
    └── types/ — Type definitions and shared interfaces (1 file)
└── scripts/ — Build or runtime scripts (17 files)
```
...
- **`src/schema/`** — Database or validation schemas
- **`src/scripts/`** — Build or runtime scripts
- **`src/types/`** — Type definitions and shared interfaces
- **`scripts/`** — Build or runtime scripts
...
│   ├── schema/               # 160 table schemas + barrel + registry
...
│   │   ├── field-types.ts    # moneyMinor, currencyCode, currencyCodeStrict, fxRate, qty, etc.
...
│   │   └── natural-key-immutability.ts  # NK trigger SQL generator + NK_TABLES registry
...
│   ├── bootstrap-neon.mjs          # Programmatic bootstrap runner
│   ├── bootstrap-neon.sql          # Idempotent DB prerequisite creation
│   ├── drizzle-migration-ci.mjs    # 8 offline CI gates (MIG-DRZ-01..04, Gate A/B)
│   ├── drizzle-neon-guard.mjs      # 4 online CI gates (MIG-DRZ-05a/b/c, MIG-DRZ-06)
│   ├── run-migrate.mjs             # Migration runner (@neondatabase/serverless Pool)
│   ├── patch-migration-triggers.mjs # NK immutability trigger patcher
│   ├── validate-registry.ts
│   ├── validate-migration-safety.ts
│   ├── validate-gates.ts           # Master gate validator (16 gates)
...
│       ├── org-uuid-01.ts
│       ├── pk-org-01.ts
│       ├── prereq-01.ts            # Bootstrap prerequisites gate
│       ├── reg-key-01.ts
│       └── rls-cast-01.ts          # auth.org_id()::uuid cast gate
├── drizzle/                  # Generated migrations
├── drizzle.config.ts         # Drizzle-kit config (walks up to find root .env)
├── drizzle.ci.config.ts      # Offline CI config (no DB credentials)
...
| `currencyCodeStrict(name)` | `text` NOT NULL, no default | ISO 4217 code (no silent default — S4/S11) |
...
```typescript
// drizzle.config.ts — .env discovery pattern
let dir = process.cwd();
for (let i = 0; i < 5; i++) {
...
`drizzle.ci.config.ts` is used for offline CI gates — no DB credentials needed.
