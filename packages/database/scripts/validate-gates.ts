...
function validateSessionGate01(): GateResult {
  ...
  try {
    ...
    const grepCmd = 'grep -r "import.*{.*\\(db\\|dbRo\\).*}.*from.*afenda-database" --include="*.ts" --include="*.tsx" --exclude-dir=node_modules --exclude-dir=dist';
  ...
  try {
    ...
    for (const file of sqlFiles) {
      ...
      const forbiddenPatterns = [
        { pattern: /CREATE\s+INDEX(?!.*CONCURRENTLY)/i, message: 'CREATE INDEX without CONCURRENTLY' },
        { pattern: /ALTER\s+TABLE.*ADD\s+COLUMN.*NOT\s+NULL(?!.*DEFAULT)/i, message: 'NOT NULL without DEFAULT' },
