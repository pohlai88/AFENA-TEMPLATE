...
if (mode === 'backfill') {
  ...
  for (const entry of toBackfill) {
    const sqlFile = readdirSync(drizzleDir).find(f => f.startsWith(entry.tag.split('_')[0] + '_') && f.endsWith('.sql'));
  ...
  console.log('\nDone. Now run `pnpm --filter afena-database db:migrate` for remaining migrations.');
} else if (mode === 'verify') {
  ...
  const pass = (v) => v ? '✅' : '❌';
  console.log(`  New unique index (scope):  ${pass(checks[0][0]?.indexname)} ${checks[0][0]?.indexname ?? 'MISSING'}`);
  console.log(`  Old unique index (no scope): ${pass(!checks[1][0]?.indexname)} ${checks[1][0]?.indexname ?? 'DROPPED'}`);
  console.log(`  roles_org_key_idx:         ${pass(checks[2][0]?.indexname)} ${checks[2][0]?.indexname ?? 'MISSING'}`);
  console.log(`  verb CHECK includes '*':   ${pass(checks[3][0]?.consrc?.includes("'*'"))} ${checks[3][0]?.consrc ?? 'MISSING'}`);
  console.log(`  trg_prevent_system_role:   ${pass(checks[4][0]?.tgname)} ${checks[4][0]?.tgname ?? 'MISSING'}`);
  console.log(`  prevent_system_role fn:    ${pass(checks[5][0]?.proname)} ${checks[5][0]?.proname ?? 'MISSING'}`);
  console.log(`  seed_org_defaults fn:      ${pass(checks[6][0]?.proname)} ${checks[6][0]?.proname ?? 'MISSING'}`);
} else {
