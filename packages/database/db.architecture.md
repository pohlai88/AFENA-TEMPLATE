...
> **Changelog:** v3.1 adds bootstrap-neon.sql prerequisites (auth schemas, 8 functions, 4 roles), 148-table baseline migration, RLS-CAST-01 gate (auth.org_id()::uuid enforcement), PREREQ-01 gate (bootstrap validation), MIG-DRZ-06 (full bootstrap+migrate integration test), run-migrate.mjs WebSocket runner. v2.6 ratification adds DbSession as single entrypoint, query plan stability (PLAN-01), online DDL gates (MIG-ONLINE-01), role separation enforcement, RLS policy catalog, boot-time role validation, prepared statements opt-in, table taxonomy enforcement, natural key immutability, worker hygiene gates
...
- **Schema:** 160 tables (truth: 45, control: 12, projection: 8, evidence: 15, link: 7, domain: 73)
- **Migrations:** drizzle-kit generate + programmatic runner (`scripts/run-migrate.mjs`)
...
export const invoiceQueries = {
  byId: (id: string) => db.select().from(invoices).where(eq(invoices.id, id)),
...
const TAXONOMY_RULES = {
  truth: {
    mustHave: [
      ...
      'hasTenant',
      'hasCompositePk',
      'hasUpdatedAtTrigger_XOR_updatedAtManagedInApp',
    ],
...
export const rateColumn = (name: string) => numeric(name, { precision: 18, scale: 8 }).notNull();
...
export const unitCountColumn = (name: string) => bigint(name, { mode: 'bigint' }).notNull();
...
export const invoices = pgTable(
  ...
  (table) => ({
    ...
    docNoUnique: uniqueIndex('invoices_org_docno_uq').on(table.orgId, table.docNo),
...
export const items = pgTable(
  ...
  (table) => ({
    ...
    // SKU unique per org
    skuUnique: uniqueIndex('items_org_sku_uq').on(table.orgId, table.sku),
  }),
);
```
...
export const moneyMinor = (name: string) => bigint(name, { mode: 'bigint' }).notNull();
...
export const customFields = pgTable(
  ...
  (table) => ({
    ...
    uniqueFieldKey: uniqueIndex('custom_fields_unique_key').on(
      table.orgId,
      table.entityType,
      table.fieldKey,
    ),
...
export const customFieldValues = pgTable(
  ...
  (table) => ({
    ...
    uniqueValue: uniqueIndex('cfv_unique_value').on(
      table.orgId,
      table.entityType,
      table.entityId,
      table.fieldKey,
...
export const standardIndexes = (table: PgTable) => ({
  orgCreated: index(`${table.name}_org_created_idx`).on(table.orgId, desc(table.createdAt)),

  orgStatus: index(`${table.name}_org_status_idx`).on(table.orgId, table.status),
...
cat packages/database/drizzle/NNNN_migration_name.sql

# 3. Run offline CI gates
pnpm --filter afenda-database db:drizzle:ci

# 4. Apply to Neon (programmatic runner — NOT drizzle-kit CLI)
pnpm --filter afenda-database db:migrate

# 5. Verify schema consistency
pnpm --filter afenda-database db:check
```
...
export async function validateQueryPlan(
  ...
  const plan = await db.execute(sql`EXPLAIN (FORMAT JSON, VERBOSE) ${sql.raw(query)}`, params);
  ...
  function checkPlanNode(node: any, violations: string[]) {
    ...
    if (node['Node Type'] === 'Seq Scan') {
      ...
      const isLargeTable = ['invoices', 'audit_logs', 'entity_versions', 'contacts'].includes(
        tableName,
      );
...
export async function archiveOldAuditLogs(beforeDate: Date, r2Bucket: R2Bucket) {
  // 1. Export to JSONL
  const rows = await db.select().from(auditLogs).where(lt(auditLogs.createdAt, beforeDate));
...
export async function listInvoices(orgId: string, limit: number) {
  return runQuery('invoices.list', 2, () =>
    db.select().from(invoices).where(eq(invoices.orgId, orgId)).limit(limit),
  );
}
...
| **Gate NUM-01**         | Money type enforcement                                                                | CI scans for _\_amount/_\_total/\*\_price and validates bigint usage                  |
| **Gate NUM-02**         | Rate type enforcement                                                                 | CI scans for _\_rate/_\_percent and validates numeric(18,8) usage                     |
...
| **Gate RLS-CAST-01**    | auth.org_id() UUID cast                                                               | CI fails if `auth.org_id()` in RLS policy lacks `::uuid` cast                         |
| **Gate PREREQ-01**      | Bootstrap prerequisites                                                               | CI validates bootstrap-neon.sql covers all required schemas, functions, roles, grants |
...
export async function rebuildSearchDocuments(orgId: string) {
  ...
  await db.delete(searchDocuments).where(eq(searchDocuments.orgId, orgId));

  // ✅ CORRECT: Explicit org_id in INSERT
  await db.insert(searchDocuments).values(docs.map((d) => ({ ...d, orgId })));
...
export async function rebuildProjection(orgId: string) {
  ...
  await workerDb.transaction(async (tx) => {
    ...
    await tx.delete(searchDocuments).where(eq(searchDocuments.orgId, orgId));

    // Rebuild from truth tables (also filtered by org_id)
    const truthData = await tx.select().from(invoices).where(eq(invoices.orgId, orgId));
...
const invoice = await db.select().from(invoices).where(eq(invoices.id, userInput)); // Automatically parameterized
...
const invoice = await db.execute(sql`SELECT * FROM invoices WHERE id = ${sql.placeholder('id')}`, {
  id: userInput,
});
```
...
The database package DTS (type) build can take ~30–40s due to the large schema barrel (`schema/index.ts`). To measure:

```bash
pnpm --filter afenda-database build:measure
```

**Future optimization:** Split schema exports by domain (e.g. `@afenda/database/schema/core`, `schema/finance`) for faster incremental builds. Apps that only need a subset could import from domain-specific paths.
