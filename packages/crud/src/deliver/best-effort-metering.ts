 ...
 *
 * Phase 6: migrated from bare `db` import to `createDbSession()` so all
 * writes carry an org-scoped RLS context via SET LOCAL (G-CRUD-03).
 */

import { createDbSession, orgUsageDaily, sql } from 'afenda-database';

/**
 * Lazy metering session — created per-call with a service identity.
 * Metering writes go to the primary and do not need retry logic.
 */
function meteringSession(orgId: string) {
  return createDbSession({ orgId, userId: '__metering__', role: 'worker' });
}
