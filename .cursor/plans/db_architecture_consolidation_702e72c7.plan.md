...
| `last_ratified`        | *(set on ratification)*                                                              |
| `ratified_by`          | *(PR approver / arch owner)*                                                         |
...
**Documentation fragmentation:** DB architecture is spread across 6+ files:

- [database.architecture.md](.architecture/database.architecture.md) — auto-generated overview
- [db.schema.governance.md](.architecture/db.schema.governance.md) — ~1700 lines, schema design, column helpers, custom fields
- [drizzle.utilization.plan.md](.architecture/drizzle.utilization.plan.md) — Drizzle automation phases
- [erp-architecture-validation.md](.architecture/erp-architecture-validation.md) — validation report (~70% aligned)
- [afenda-spec-evaluation.md](.architecture/afenda-spec-evaluation.md) — ratification evaluation (~45% aligned)
- [neon-mcp.usage.md](.architecture/neon-mcp.usage.md) — Neon MCP usage guide

**Drizzle gaps:** Schema barrel done; relations partial; TABLE_REGISTRY/HANDLER_REGISTRY manual; no composite PK/FK; no prepared statements; no batching guidance; flat schema (no module folders).

**Data serialization:** Not documented. No explicit API-to-DB type coercion layer (dates, UUIDs, JSONB). Drizzle schema types exist but API boundary handling is implicit.

**Data sanitization:** Only `stripSystemColumns` in [packages/crud/src/sanitize.ts](packages/crud/src/sanitize.ts) (K-11). No entity-specific allowlist (entity-new.ts `pickAllowed` is TODO); no XSS/HTML escaping; custom field validation exists for `custom_data` only.
...
| ---------- | -------------------------------------- | ------------------------------------------------------ | -------------- | --------- | --------------------- | ---------------------------------------------------------------------------------------------- |
| GAP-DB-001 | PK (id) only                           | PK (org_id, id) for truth tables                       | Data integrity | P2        | schema-lint rule      | All truth tables have composite PK; schema-lint passes                                         |
| GAP-DB-002 | FKs sparse on domain                   | Every *_id has FK unless whitelisted                   | Data integrity | P2        | schema-lint rule      | No *_id without FK except whitelist; schema-lint passes                                        |
| GAP-DB-003 | stock_balances writable                | REVOKE UPDATE/DELETE; projection-only                  | Data integrity | P2        | RLS test              | REVOKE in migration; RLS test passes                                                           |
| GAP-DB-004 | No outbox + search_documents           | Outbox + incremental search worker                     | Scale          | P3        | N/A                   | Outbox + worker deployed; search_documents populated                                           |
| GAP-DB-005 | RLS_TABLES hand-maintained             | Generated from schema                                  | Ops            | P3        | CI script             | RLS_TABLES generated from schema; cross-tenant test imports it                                 |
| GAP-DB-006 | No data serialization layer            | Typed Boundary Layer (SER-01)                          | Data integrity | P2        | serialization.test.ts | Canon serialization module exists; handlers receive MutationSpec only                          |
| GAP-DB-007 | No schema-derived allowlist            | SAN-01: writable cols from schema                      | Security       | P2        | sanitize.test.ts      | All handlers use schema-derived writable columns; pickAllowed removed from entity-new template |
| GAP-DB-008 | doc_postings lacks doc_version         | Add column; unique (org, type, id, version)            | Data integrity | P2        | migration             | doc_version column added; unique constraint in place                                           |
| GAP-DB-009 | No prepared statements for hot queries | Use .prepare() where driver supports; else batch/cache | Performance    | P2        | Perf benchmark        | Hot paths use .prepare() or batch/cache; benchmark confirms benefit                            |
...
| **Batch API**           | Not used         | `db.batch([...])` for multi-statement reads; reduces round trips | [batch-api](https://orm.drizzle.team/docs/batch-api)         |
...
**Validated by:** ESLint INVARIANT-01, cross-tenant tests, future: posting-path test
...
| Gate 2 | Identity rules (truth tables (org_id, id) or exception)                                                                                                 | Manual                | schema-lint       |
| Gate 3 | FK coverage (*_id has FK unless whitelisted)                                                                                                            | Manual                | schema-lint       |
| Gate 4 | Postable docs registered                                                                                                                                | schema-lint.config.ts | schema-derived    |
| Gate 5 | Append-only tables immutable                                                                                                                            | REVOKE in migrations  | migration lint    |
| Gate 6 | Projection tables no app writes                                                                                                                         | Manual                | RLS test          |
| Gate 7 | Registry drift (TABLE_REGISTRY, HANDLER_REGISTRY)                                                                                                       | Manual                | schema-driven gen |
...
**Validated by:** `pnpm db:lint`, cross-tenant-rls.test.ts
...
- **Code changes** — Plan mode; no edits to packages/crud, packages/database
- **Composite PK migration** — Documented as GAP-DB-001; implementation separate
- **Outbox + search_documents** — Documented as GAP-DB-004; implementation separate
- **stock_balances REVOKE** — Documented as GAP-DB-003; implementation separate
