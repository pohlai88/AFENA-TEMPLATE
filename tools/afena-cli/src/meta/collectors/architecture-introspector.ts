...
function introspectPackage(
  ...
  try {
    const pkg: Record<string, any> = JSON.parse(readFileSync(pkgJsonPath, 'utf-8'));
    const name: string = (pkg.name as string) ?? basename(pkgDir);
    const description: string = (pkg.description as string) ?? '';
