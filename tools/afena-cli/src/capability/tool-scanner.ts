...
import { PackageJsonSchema, safeReadJsonIfExists } from '../core/parse-json';
...
export function scanTools(
  ...
  for (const toolFile of toolDirs.sort()) {
    ...
    const segments = [...toolDir.split('/'), 'package.json'];
    const pkg = safeReadJsonIfExists(repoRoot, segments, PackageJsonSchema);
