 ...
 * Provides standardized, clear, and clean reports for all afena-cli commands
 */
...
export function generateReport(report: CommandReport): string {
  ...
  lines.push(pc.bold(`  AFENA CLI REPORT: ${report.command.toUpperCase()}`));
  lines.push(pc.bold('═'.repeat(80)));
  lines.push('');
...
export function generateMarkdownReport(report: CommandReport): string {
  ...
  lines.push('---');
  lines.push('');
  lines.push(`*Generated by afena-cli at ${report.timestamp}*`);
  lines.push('');
