...
<!-- Signature: sha256:f01cb386d64608b1de418f6df7d5c49d175dff18b740ab7613cb2601fad9ecd1 -->
# @afenda/cli
...
✅ Has Tests • 📚 Has Documentation • 📁 12 directory areas
...
└── src/
    ├── bundle/ (1 file)
    ├── capability/ — Capability or feature modules (31 files)
    ├── checks/ (2 files)
    ├── core/ — Core utilities and base types (16 files)
    ├── discovery/ (2 files)
    ├── docs/ — Documentation and guides (1 file)
    ├── doctor/ (2 files)
    ├── domain/ — Domain logic and entities (2 files)
    ├── project/ (8 files)
    ├── proposal/ (5 files)
    └── readme/ — README generation templates and renderers (7 files)
```
...
- `src/checks/index.ts` — barrel export
- `src/doctor/index.ts` — barrel export
- `src/domain/index.ts` — barrel export
- `src/project/index.ts` — barrel export
- `src/proposal/index.ts` — barrel export
...
| `lint:ci` | `eslint src --ext .js,.jsx,.ts,.tsx --cache --rule 'import/no-cycle: error'` |
...
# 🔧 afenda CLI - Enterprise Command Reference
...
[Architecture Docs](#-architecture-doc-generation) •
[Workflows](#-common-workflows) • [CI/CD](#-cicd-integration) •
...
✅ **Capability Governance** - Validate and generate capability documentation\
✅ **README Generation** - Auto-generate consistent package READMEs\
✅ **Metadata Validation** - Ensure quality standards (VIS-00 to VIS-04)\
✅ **Architecture Documentation** - Generate architecture docs from code\
✅ **Housekeeping Automation** - Run invariant checks (E1-E7, H00-H02)

### Design Principles

🔧 **Declarative Configuration** - All commands defined in `report-config.ts`\
📊 **Consistent Reporting** - Unified CLI and Markdown output\
⚡ **Fast Execution** - Optimized for monorepo scale (38 packages, 83K+ LOC)\
🎨 **Professional UX** - Color-coded output with status indicators
...
```
🔍 Scanning codebase...
✅ Found 12 capability surfaces
✅ Found 14 UI surfaces
✅ Scanned 38 packages, 175 tables, 83,229 LOC

📊 Summary:
   24 covered | 8 orphaned | 0 phantom | 2 excepted

✅ Wrote .afenda/capability.ledger.json
✅ Wrote .afenda/capability.matrix.md
✅ Wrote .afenda/codebase.manifest.json
✅ Wrote .agents/context/capability-map.md
```
...
✅ VIS-00: Package metadata (38/38 packages)
✅ VIS-01: Capability annotations (24/24 surfaces)
⚠️  VIS-02: Surface coverage (16/24 covered, 8 orphaned)
✅ VIS-03: UI documentation (14/14 surfaces)
✅ VIS-04: Cross-package refs (100% valid)
...
```
┌─────────────────────┬────────────┬──────────┬─────────┐
│ Capability          │ Surface    │ UI       │ Status  │
├─────────────────────┼────────────┼──────────┼─────────┤
│ accounting/gl       │ ✅ GL.ts   │ ✅ Page  │ Covered │
│ crm/contacts        │ ✅ CRM.ts  │ ❌       │ Orphaned│
│ workflow/approval   │ ❌         │ ✅ Page  │ Phantom │
└─────────────────────┴────────────┴──────────┴─────────┘
```
...
```
🔄 Syncing README auto-gen blocks...

✅ packages/accounting/README.md (updated auto-gen block, 142 → 156 lines)
✅ packages/canon/README.md (no changes needed)
⚠️  packages/crm/README.md (missing auto-gen markers, skipped)
...
- ✅ Required sections exist (Overview, Usage, Architecture)
...
✅ E1: Package exports (38/38 valid)
✅ E2: No circular imports (0 cycles found)
✅ E3: Naming conventions (all packages compliant)
⚠️  E4: Package.json fields (2 packages missing "repository")
✅ E5: Dependency versions (all catalog:* valid)
✅ E6: Export documentation (100% match)
✅ E7: Type definitions (all packages have .d.ts)
✅ H00: Dependency health (no deprecated packages)
✅ H01: Security (0 vulnerabilities)
✅ H02: Licenses (all MIT/Apache-2.0)
      ...
      - name: Check for outdated READMEs
        run: |
          if ! git diff --quiet; then
            echo "❌ READMEs are out of sync. Run 'pnpm afenda readme gen'"
            exit 1
          fi
```
...
```bash
$ pnpm afenda meta gen
✅ Wrote .afenda/capability.ledger.json (0 KB)
```
...
```bash
$ pnpm afenda meta check
⚠️ VIS-02: Surface coverage (8 orphaned surfaces)
Exit code: 1
```
...
```bash
$ pnpm afenda readme sync
⚠️ packages/accounting/README.md has merge conflicts in auto-gen block
```
...
[Tools README](../README.md) • [Development Guide](../GUIDE.md) •
...
[⬆ Back to Top](#-afenda-cli---enterprise-command-reference)
