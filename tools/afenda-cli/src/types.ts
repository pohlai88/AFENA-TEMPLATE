...
export interface ResolvedExecParams {
  ...
  shell: boolean;
  /** Timeout in ms. Default 300_000 (5 min) for long commands when not specified. */
  timeout?: number;
  /** Max retries for transient failures. Default 0. */
  maxRetries?: number;
  /** Delay in ms before first retry. Exponential backoff applied. Default 1000. */
  retryDelayMs?: number;
}
  ...
  .object({
    ...
    structure: z
      .object({
        ...
        isEmpty: z.boolean(),
        /** Top-level directories (e.g. src, scripts, engine) */
        topLevelDirs: z.array(z.string()).optional(),
        /** Short description per directory name for README layout */
        dirDescriptions: z.record(z.string(), z.string()).optional(),
        /** File count per directory for overview */
        fileCountByDir: z.record(z.string(), z.number()).optional(),
      })
      .optional(),
