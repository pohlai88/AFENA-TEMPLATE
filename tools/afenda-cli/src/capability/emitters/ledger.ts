...
/** Returns true if advisory package (packages/advisory) exists in repo. */
function advisoryPackageExists(repoRoot: string): boolean {
  return existsSync(join(repoRoot, 'packages', 'advisory'));
}

/**
 * Generate the capability ledger from scan results + exceptions.
 * When repoRoot is provided, excludes system.advisory.* entries if packages/advisory does not exist.
 */
export function generateLedger(
  ...
  const excludeAdvisory = repoRoot != null && !advisoryPackageExists(repoRoot);
  ...
  for (const [key, descriptor] of Object.entries(CAPABILITY_CATALOG)) {
    if (excludeAdvisory && key.startsWith('system.advisory.')) continue;
