...
export function runReadmeCommand(
  ...
  const { mode, repoRoot, config, dryRun, json: jsonOutput } = options;
  ...
  if (!options.packages || options.packages.length === 0) {
    const rootAction = generateRootReadme(repoRoot, pkgDirs, mode, dryRun, jsonOutput);
  ...
  if (mode === 'gen' && (!options.packages || options.packages.length === 0)) {
    generateArchitectureDocuments(repoRoot, dryRun, jsonOutput);
  }

  // Print summary (skip when --json)
  if (!jsonOutput) printSummary(mode, result, dryRun);
...
function generateRootReadme(
  ...
  if (dryRun && jsonOutput) return readmeExists ? 'updated' : 'generated';
...
function generateArchitectureDocuments(
  ...
  if (dryRun && jsonOutput) return;

  const written = writeArchitectureDocs(repoRoot, docs);
  if (!jsonOutput) log.success(`Generated ${written.length} architecture doc(s): ${written.join(', ')}`);
}
