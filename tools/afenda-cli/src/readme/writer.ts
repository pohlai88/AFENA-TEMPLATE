...
export function writeReadme(
  ...
  if (!safeExists(repoRoot, ...pkgDir.split('/'), 'README.md')) {
    safeWriteFile(repoRoot, `${autogenBlock}\n`, ...pkgDir.split('/'), 'README.md');
    return { action: 'created' };
  }

  const existing = safeReadFile(repoRoot, ...pkgDir.split('/'), 'README.md');
  ...
  if (startCount === 1 && endCount === 1) {
    ...
    if (startIdx >= 0 && endIdx > startIdx) {
      ...
      safeWriteFile(repoRoot, updated, ...pkgDir.split('/'), 'README.md');
...
export function validateReadme(
  ...
  const content = safeReadFile(repoRoot, ...pkgDir.split('/'), 'README.md');
